...
- `/app/src/design/fonts/` — accepted web fonts. Reference files that are present via `@font-face` in your own CSS and use the resulting `font-family` consistently. Restricted Apple system-font files are omitted before generation; never recreate, download, or embed a missing SF-family or New York font. Preserve that appearance with a system stack instead: use `system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif` for SF Pro/Compact, `ui-monospace, "SFMono-Regular", Menlo, Monaco, Consolas, monospace` for SF Mono, or `ui-serif, Georgia, serif` for New York. Do not link to external font CDNs (Google Fonts, Bunny, etc.) when fonts are present here.
...
- **Theme CSS may deliberately pin one light/dark mode, and that wins.** The default rule is to follow the host's mode (complete `:root` light values, complete `.dark` values). If the organization's CSS defines only one set of values, or its instructions say the app must always render on the brand's dark (or light) surface, honour that — brand consistency beats mode-following. Don't "fix" a single-mode theme by inventing the other mode's colours.
