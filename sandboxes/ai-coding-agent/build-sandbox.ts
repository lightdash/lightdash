...
async function main() {
    try {
        ...
        let logsOffset = 0;
        // A transient network/API hiccup while polling shouldn't fail a build
        // that's actually progressing; tolerate a few consecutive poll errors
        // before giving up.
        ...
        while (true) {
            let status;
            try {
                status = await Template.getBuildStatus(info, { logsOffset });
                consecutivePollErrors = 0;
            } catch (pollErr: any) {
                consecutivePollErrors++;
                if (consecutivePollErrors > MAX_POLL_ERRORS) throw pollErr;
                console.warn(
                    `Failed to fetch build status (${consecutivePollErrors}/${MAX_POLL_ERRORS}), retrying: ${
