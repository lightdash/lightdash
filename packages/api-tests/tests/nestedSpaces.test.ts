import { generateSlug, SEED_PROJECT } from '@lightdash/common';
...
describe('Lightdash API tests for my own private spaces as admin', () => {
    ...
    it('Should not create duplicate slugs in the same project', async () => {
        ...
        const [res1, res2] = await Promise.all([
            admin.post<Body<{ slug: string; uuid: string }>>(
        ...
        expect(
            new Set([res1.body.results.slug, res2.body.results.slug]),
        ).toEqual(
            new Set([generateSlug(spaceName), `${generateSlug(spaceName)}-1`]),
        );
    });
