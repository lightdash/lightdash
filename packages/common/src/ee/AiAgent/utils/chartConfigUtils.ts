...
const getBarChartConfig = ({
    ...
    const xDimension = queryTool.queryConfig.dimensions[0];
    const { breakdownByDimension: breakdownDimension } = queryTool.queryConfig;

    const { metrics: queryMetrics } = metricQuery;
    let metrics: (string | PivotReference)[] = queryMetrics;

    // If there's a breakdown dimension, pivot the data
