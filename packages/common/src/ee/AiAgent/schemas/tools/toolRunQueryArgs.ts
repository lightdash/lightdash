...
// Uuid-enriched shape the server writes into artifacts after resolving the
// slug (mirrors the saved-chart DataAppVizChart shape). Parse-only — never
// advertised to the model.
const chartConfigCustomChartTypePersistedSchema = z.object({
    dataAppVizUuid: z.string(),
...
// Advertised agent contract: builtin viz config | custom chart type.
const chartConfigAdvertisedSchema = z
    .union([chartConfigBuiltinSchema, chartConfigCustomChartTypeSchema])
    .nullable();

// Wide union used by the parse/persist schemas: accepts everything ever
// advertised plus the uuid-enriched artifact shape.
const chartConfigSchema = z
    .union([
        chartConfigBuiltinSchema,
        chartConfigCustomChartTypeSchema,
        chartConfigCustomChartTypePersistedSchema,
    ])
    .nullable();
...
>;
export type ToolRunQueryCustomChartTypePersistedConfig = z.infer<
    typeof chartConfigCustomChartTypePersistedSchema
>;
export type ToolRunQueryChartConfig = z.infer<typeof chartConfigSchema>;

// Compile-time guard: the persisted branch must stay in lockstep with the
// saved-chart DataAppVizChart shape (render and save-to-chart share one path).
type AssertMutuallyAssignable<A, B> = [A] extends [B]
    ? [B] extends [A]
        ? true
        : never
    : never;
const persistedCustomChartConfigMatchesSavedChart: AssertMutuallyAssignable<
    ToolRunQueryCustomChartTypePersistedConfig,
    DataAppVizChart
> = true;
void persistedCustomChartConfigMatchesSavedChart;
...
export const isCustomChartTypePersistedChartConfig = (
    chartConfig: ToolRunQueryChartConfig | undefined,
): chartConfig is ToolRunQueryCustomChartTypePersistedConfig =>
    !!chartConfig && 'dataAppVizUuid' in chartConfig;

// Either custom branch — the thread render path treats both the same.
export const isCustomChartTypeChartConfig = (
    chartConfig: ToolRunQueryChartConfig | undefined,
): chartConfig is
    | ToolRunQueryCustomChartTypeConfig
    | ToolRunQueryCustomChartTypePersistedConfig =>
    isCustomChartTypeSlugChartConfig(chartConfig) ||
    isCustomChartTypePersistedChartConfig(chartConfig);
...
export const toolRunQueryArgsSchemaV4 = createToolSchema()
    .extend({
        ...
        chartConfig: chartConfigAdvertisedSchema,
        mergeConfig: mergeConfigSchema.default(null),
    })
    .build();
...
// Agent view of V2 for merge-disabled runtimes: advertises only the shapes
// the model may author (builtin | custom slug), not the persisted branch.
export const toolRunQueryArgsSchemaV2Advertised =
    toolRunQueryArgsSchemaV2.extend({
        chartConfig: chartConfigAdvertisedSchema,
    });
