...
const toolRunQueryArgsSchema = createToolSchema()
    .extend({
...
const toolRunQueryArgsSchemaTransformed = toolRunQueryArgsSchema
    .extend({
        customMetrics: customMetricsSchema
            .default(null)
...
const toolRunQueryOutputSchema = z.object({
    result: z.string(),
    metadata: baseOutputMetadataSchema,
});

/** MCP App UI resource backing the run_metric_query interactive chart. */
export const RUN_METRIC_QUERY_CHART_RESOURCE_URI =
    'ui://run-metric-query/chart.html';

// Agent name `runQuery` is exposed over MCP as `run_metric_query` (name
// override). The MCP variant also links the interactive chart App resource.
...
`,
    ...
    mcp: {
        ...
        meta: { ui: { resourceUri: RUN_METRIC_QUERY_CHART_RESOURCE_URI } },
    },
});
