...
export const MCP_QUERY_TIMING_NOTE = `Each run_sql, run_metric_query, and get_query_result call waits up to ${MCP_QUERY_SYNC_WAIT_LABEL}. If unfinished, call get_query_result again with queryUuid after nextPollAfterMs. Warehouse execution timeout comes from the Lightdash warehouse connection, not MCP. Client/transport timeouts are retryable with the same queryUuid.`;
...
- Running: structuredContent.result contains { status: "running", queryUuid, nextPollAfterMs, heartbeatAt }. ${MCP_QUERY_STRUCTURED_RUNNING_NOTE}
...
- Failed/cancelled/expired: structuredContent.result contains { status, queryUuid, error }.`;
