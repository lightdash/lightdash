...
const getToolListWarehouseTablesDescription = ({
    name,
    findExploresName,
    findFieldsName,
    runSqlName,
...
Purpose:
List physical tables available in the connected data warehouse. Use this BEFORE writing a ${runSqlName} call when you need to discover the correct schema or table name for a table that isn't already exposed via an explore.
...
- You're looking for a raw / staging / seed table that isn't surfaced by ${findExploresName}.
- You're trying to confirm a schema name before writing SQL.

Do NOT use:
- For column/field discovery — use ${findFieldsName} on the relevant explore instead.
- To run ad-hoc queries — use ${runSqlName}.
...
const toolListWarehouseTablesOutputSchema = z.object({
    result: z.string(),
    metadata: z.object({
...
export const listWarehouseTablesTool = defineTool({
    canonicalName: 'listWarehouseTables',
    title: 'List Warehouse Tables',
    contexts: ['agent'] as const,
    description: {
        agent: ({ name }) =>
            getToolListWarehouseTablesDescription({
    ...
    outputSchema: toolListWarehouseTablesOutputSchema,
});

export type ToolListWarehouseTablesArgs = ToolInput<
    typeof listWarehouseTablesTool,
    'agent'
>;
export type ToolListWarehouseTablesOutput = ToolOutput<
    typeof listWarehouseTablesTool
>;
