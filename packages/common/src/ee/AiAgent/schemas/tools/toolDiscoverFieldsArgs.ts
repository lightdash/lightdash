...
const discoverFieldsInputSchema = z.object({
    userQuery: z
        .string()
        .describe(
...
export const discoverFieldsTool = defineTool({
    canonicalName: 'discoverFields',
    title: 'Discover Fields',
    contexts: ['agent'] as const,
    buildInputSchemas: {
        agent: () => discoverFieldsInputSchema,
    },
    outputSchema: discoverFieldsResultSchema,
});

export type DiscoverFieldsInput = ToolInput<typeof discoverFieldsTool, 'agent'>;
export type DiscoverFieldsResult = ToolOutput<
    typeof discoverFieldsTool
>['handoff'];
