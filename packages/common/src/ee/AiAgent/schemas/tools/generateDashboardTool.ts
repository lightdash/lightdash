...
const TOOL_DASHBOARD_V2_DESCRIPTION = `
...
const dashboardV2VisualizationSchema = runQueryTool.inputSchema;

export type DashboardV2Visualization = z.infer<
    typeof dashboardV2VisualizationSchema
>;

const toolDashboardV2ArgsSchema = createToolSchema()
    .extend({
...
const toolDashboardV2ArgsSchemaTransformed =
    toolDashboardV2ArgsSchema.transform((data) => ({
        ...data,
        visualizations: data.visualizations.map((viz) =>
            runQueryTool.inputSchemaTransformed.parse(viz),
        ),
    }));

const toolDashboardV2OutputSchema = z.object({
    result: z.string(),
    metadata: baseOutputMetadataSchema,
});

export const generateDashboardTool = defineTool({
    name: 'generateDashboard',
    title: 'Generate Dashboard',
    description: TOOL_DASHBOARD_V2_DESCRIPTION,
    availability: 'agent',
    inputSchema: toolDashboardV2ArgsSchema,
    inputSchemaTransformed: toolDashboardV2ArgsSchemaTransformed,
