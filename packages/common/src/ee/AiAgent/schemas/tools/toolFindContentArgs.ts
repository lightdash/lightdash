...
- verifiedOnly=true searches only admin-verified charts/dashboards (spaces omitted). Verification is a status, not text — search topic keywords, never the word "verified". If nothing verified matches, unverified results are returned, marked as such; verified content may still exist under other terms, so never claim the project has none from one query.
