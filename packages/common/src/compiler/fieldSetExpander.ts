...
export const FIELD_SET_LIMITS = {
    MAX_SET_SIZE: 100,
    MAX_TOTAL_EXPANDED: 500,
} as const;
...
function expandSetReferences(
    ...
    for (const inclusion of inclusions) {
        if (inclusion.endsWith('*')) {
            ...
            // Validate set size
            if (setDefinition.fields.length > FIELD_SET_LIMITS.MAX_SET_SIZE) {
                throw new CompileError(
                    `Set "${setName}" contains ${setDefinition.fields.length} fields, which exceeds the maximum size of ${FIELD_SET_LIMITS.MAX_SET_SIZE}`,
                );
            }
...
export function expandFieldsWithSets(
    ...
    // Validate total size
    if (result.length > FIELD_SET_LIMITS.MAX_TOTAL_EXPANDED) {
        throw new CompileError(
            `Expanded field list exceeds maximum size of ${FIELD_SET_LIMITS.MAX_TOTAL_EXPANDED} fields. Got ${result.length} fields.`,
        );
    }
