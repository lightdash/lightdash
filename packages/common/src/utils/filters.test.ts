...
describe('getOverriddenChartFilterRuleIds', () => {
    test('includes only the overridden top-level rule when a nested rule targets the same date field', () => {
        ...
        const result = getOverriddenChartFilterRuleIds({
            chartFilters,
            dashboardFilters: appliedDashboardFilters,
            explore: mockExplore,
        });

        expect(result).toEqual(new Set(['month-filter']));
    });

    test('preserves exact matching for non-time filters', () => {
        const chartFilters: Filters = {
            dimensions: {
                id: 'chart-filter-group',
                and: [
        ...
        expect(result.has('status-filter')).toBe(true);
    });

    test('does not mark unrelated chart filters as overridden', () => {
        const chartFilters: Filters = {
            dimensions: {
                id: 'chart-filter-group',
                and: [
        ...
        expect(result.has('status-filter')).toBe(false);
    });
});
