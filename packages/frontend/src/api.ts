...
import { type LocalStorageEmbed } from './ee/providers/Embed/EmbedProvider';
...
function finalizeUrl(url: string, embed: LocalStorageEmbed | null): string {
...
export const lightdashApi = async <T extends ApiResponse['results']>({
    ...
    const embed = getFromLocalStorage<LocalStorageEmbed>(EMBED_KEY);
