...
The roadmap uses the shared Settings page header and search input, with Status, Priority, and Interest facets on the left and board/table controls on the right. The Interest dropdown contains the “Only our interests” switch. Backlog and planned items share the customer-facing “Backlog” status in the board, table, and filters. Filters apply before pagination; project ticket filters are separate from the main board filters.
