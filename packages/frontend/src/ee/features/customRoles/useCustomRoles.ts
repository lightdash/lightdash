...
export const useCustomRoles = () => {
    ...
    const listRoles = useQuery<RoleWithScopes[], ApiError>({
        ...
        queryFn: async () => {
            ...
            const response = await lightdashApi<RoleWithScopes[]>({
                method: 'GET',
                url: `/orgs/${organization.organizationUuid}/roles`,
                version: 'v2',
            });
            // Filter to only show custom roles (user-created), not system roles
            return response.filter((role) => role.ownerType === 'user');
        },
