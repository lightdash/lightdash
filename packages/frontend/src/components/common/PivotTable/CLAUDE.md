...
```json
{
  "headerValues": [
    // Row 0: Month values
    [
      { "type": "value", "fieldId": "orders_order_date_month", "value": { "raw": "2025-01-01T00:00:00Z", "formatted": "2025-01" }, "colSpan": 3 },
      { "type": "value", "fieldId": "orders_order_date_month", "value": { "raw": "2024-06-01T00:00:00Z", "formatted": "2024-06" }, "colSpan": 3 }
    ],
    // Row 1: Shipping method values
    [
      { "type": "value", "fieldId": "orders_shipping_method", "value": { "raw": "standard", "formatted": "standard" }, "colSpan": 1 },
      { "type": "value", "fieldId": "orders_shipping_method", "value": { "raw": "express", "formatted": "express" }, "colSpan": 1 }
      // ... etc
    ]
  ]
}
...
{
  "pivotConfig": {
    "pivotDimensions": ["orders_order_date_month", "orders_shipping_method"],
    "metricsAsRows": true,
    "columnOrder": [
...
```json
{
  "pivotColumnInfo": [
    ...
    { "fieldId": "orders_order_date_month__orders_shipping_method__0", "baseId": "orders_shipping_method" },
    { "fieldId": "orders_order_date_month__orders_shipping_method__1", "baseId": "orders_shipping_method" }
    // ... etc
  ]
}
```
...
```json
{
  "allCombinedData": [
    // Row 0
    {
      "label-0": { "value": { "raw": "Total order amount", "formatted": "Total order amount" } },
      "orders_order_date_month__orders_shipping_method__0": { "value": { "raw": 1, "formatted": "$1.00" } },
      "orders_order_date_month__orders_shipping_method__1": { "value": { "raw": 27, "formatted": "$27.00" } }
      // ... all 30 columns
    },
    // Row 1
    {
      "label-0": { "value": { "raw": "Total completed order amount", "formatted": "..." } }
      // ... all columns
    }
  ]
}
...
**Location:** `packages/common/src/types/pivot.ts`
...
type TitleField = null | { fieldId: string; direction: 'index' | 'header' };

export type PivotColumn = {
  fieldId: string; // Unique flattened ID
  baseId: string | undefined; // Original field ID (last pivot dim when metricsAsRows)
  underlyingId: string | undefined;
  columnType: string | undefined; // 'label' | 'indexValue' | 'rowTotal'
};
