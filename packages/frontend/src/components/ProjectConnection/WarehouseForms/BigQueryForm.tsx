...
const BigQueryForm: FC<{
    ...
    const {
        ...
        error: datasetsError,
    } = useBigqueryDatasets(isAuthenticated, debouncedProject);
    ...
    const savedConnection = useMemo(() => {
        return savedProject?.warehouseConnection?.type ===
    ...
    const defaultAuthenticationType =
        savedProject && savedConnection
            ? savedConnection?.authenticationType ||
              BigqueryAuthenticationType.PRIVATE_KEY
            : BigqueryAuthenticationType.SSO;

    useEffect(() => {
        // Reset authenticationType when savedConnection changes, trigger only when savedConnection is loaded
        if (savedConnection)
            form.setFieldValue(
    ...
    }, [savedConnection]); // eslint-disable-line react-hooks/exhaustive-deps
