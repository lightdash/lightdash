...
export const DucklakeSchemaInput: FC<{
    disabled: boolean;
}> = ({ disabled }) => {
...
const CATALOG_TYPE_OPTIONS = [
    { value: DucklakeCatalogType.POSTGRES, label: 'PostgreSQL (recommended)' },
    { value: DucklakeCatalogType.SQLITE, label: 'SQLite (local file)' },
    { value: DucklakeCatalogType.DUCKDB, label: 'DuckDB (local file)' },
];

const DATA_PATH_TYPE_OPTIONS = [
    { value: DucklakeDataPathType.S3, label: 'S3 / S3-compatible' },
...
const DucklakeForm: FC<{
    ...
    if (warehouse?.type !== WarehouseTypes.DUCKLAKE) {
        throw new Error('Ducklake form is not used for this warehouse type');
    }
            ...
            <FormSection isOpen={isOpen} name="advanced">
                <Stack mt="sm">
                    <NumberInput
                        ...
                        label="Threads"
                        description="Number of threads for DuckDB to use."
                        defaultValue={1}
