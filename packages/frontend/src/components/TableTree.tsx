...
const NodeItemButtons: FC<{
    ...
    onOpenSourceDialog: (source: Source) => void;
    isHovered: boolean;
    setCustomMetrics?: Dispatch<SetStateAction<Metric[]>>;
}> = ({ node, onOpenSourceDialog, isHovered, setCustomMetrics }) => {
    ...
    const createCustomMetric = (metric: any, type: MetricType) => {
        const customMetricData: Metric[] = [
            {
                name: metric.name,
                label: `${friendlyName(metric.type)} of ${metric.label}`,
                table: metric.table,
                tableLabel: metric.tableLabel,
                sql: metric.sql,
                description: `${friendlyName(metric.type)} of ${
                    metric.label
                } on the table ${metric.tableLabel}`,
        ...
        // @ts-ignore
        setCustomMetrics(customMetricData);
    };
