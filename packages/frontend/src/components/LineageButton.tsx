...
const Content = () => {
    ...
    Object.entries(table.lineageGraph).forEach(([modelName, dependencies]) => {
        dependencies.forEach(dependency => {
            if (showAll || table.name === modelName || table.name === dependency.name) {
    ...
    return (
        <div style={{display: 'flex', padding: 5, flexDirection: 'column'}}>
            ...
            <div style={{display: 'flex', justifyContent: 'flex-end'}}>
                <Button icon={showAll ? 'zoom-out' : 'zoom-in'} text={showAll ? 'Show direct dependencies' : 'Show all'} minimal={true} onClick={() => setShowAll(x => !x)}/>
            </div>
        </div>
    )
}
