...
type Props = {
    ...
    metricQuery?: MetricQuery | undefined; // Make optional for backward compat
    queryUuid?: string;
};
...
const MetricQueryDataProvider: FC<React.PropsWithChildren<Props>> = ({
    tableName,
    explore,
    metricQuery: metricQueryProp,
    queryUuid,
    children,
}) => {
    // Read metricQuery from Redux to avoid parent re-renders
    const metricQueryFromRedux = useExplorerSelector(selectMetricQuery);
    const metricQuery = metricQueryProp ?? metricQueryFromRedux;
