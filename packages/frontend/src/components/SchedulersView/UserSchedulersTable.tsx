...
import { useUserPaginatedSchedulers } from '../../features/scheduler/hooks/useScheduler';
import {
    useSchedulerFilters,
...
interface UserSchedulersTableProps {
...
const UserSchedulersTable: FC<UserSchedulersTableProps> = ({
    getSlackChannelName,
    ...
    // Get the projectUuid from the first scheduler item for fetching project details
    // All schedulers may have different projects, so we fetch each project as needed
    const firstProjectUuid = flatData[0]?.projectUuid ?? undefined;
    const { data: project } = useProject(firstProjectUuid);
...
export default UserSchedulersTable;
