...
export type VirtualListItem =
  ...
  | { type: "section-header"; id: string; label: string; isExpanded: boolean };
...
const VirtualizedExploreList: FC<VirtualizedExploreListProps> = ({
  ...
  const virtualItems = useMemo((): VirtualListItem[] => {
    ...
    if (customUngroupedExplores.length > 0) {
      const isVirtualViewsExpanded = expandedSections.has("Virtual Views");
    ...
    if (preAggregateExplores.length > 0) {
      const isPreAggregatesExpanded = expandedSections.has("Pre-aggregates");
  ...
  const renderItem = useCallback(
    (item: VirtualListItem) => {
      switch (item.type) {
        ...
        case "divider":
          return <Divider key={item.id} size={0.5} c="ldGray.5" my="xs" />;
