...
describe('buildWarehouseTreeRows', () => {
    ...
    it('falls back to the sole catalog database for a Databricks connection with no catalog set', () => {
        ...
        const rows = build({
            ...
            expanded: [
                `database:${CONNECTION_ID}/hive_metastore`,
                `schema:${CONNECTION_ID}/hive_metastore/default`,
            ],
        });
        ...
        expect(rows.map((row) => row.id)).toEqual([
            `database:${CONNECTION_ID}/hive_metastore`,
            `schema:${CONNECTION_ID}/hive_metastore/default`,
            `table:${CONNECTION_ID}/hive_metastore/default/orders`,
        ]);
        expect(rows[0]).toMatchObject({ childCount: 1 });
    });

    it('inserts the real catalog key, not the DEFAULT placeholder, for a Databricks connection with no catalog set', () => {
        const databricksDatabases: WarehouseListedDatabase[] = [
            {
                name: 'DEFAULT',
                database: 'DEFAULT',
        ...
        const rows = build({
            ...
            units: { DEFAULT: loaded(databricksCatalog) },
            expanded: [
                `database:${CONNECTION_ID}/lightdash_staging`,
                `schema:${CONNECTION_ID}/lightdash_staging/jaffle`,
            ],
        });

        expect(
            rows.some(
                (row) => row.type === 'database' && row.database === 'DEFAULT',
            ),
        ).toBe(false);

        const tableRow = rows.find(
            (row): row is WarehouseTreeRow & { type: 'table' } =>
                row.type === 'table',
        );
        expect(tableRow?.identity).toEqual({
            connectionId: CONNECTION_ID,
            database: 'lightdash_staging',
            schema: 'jaffle',
            table: 'customers',
        });
        expect(qualifiedTableName(tableRow!.identity, '`')).toBe(
            '`lightdash_staging`.`jaffle`.`customers`',
        );
    });

    const motherDuckDatabases: WarehouseListedDatabase[] = [
        { name: 'my_db', database: 'my_db', schema: null, isDefault: true },
    ];
    const motherDuckCatalog: WarehouseTablesCatalog = {
        jaffle_shop: { main: { customers: {} } },
        my_db: { main: { orders: {} } },
        sample_data: { main: { movies: {} } },
        upvest_demo: { main: { accounts: {} } },
        md_information_schema: { information_schema: { tables: {} } },
    };

    it('renders every catalog database for a warehouse without listing support', () => {
        const rows = build({
            connections: [connection(motherDuckDatabases)],
            units: { my_db: loaded(motherDuckCatalog) },
        });

        expect(
            rows
                .filter((row) => row.type === 'database')
    ...
    it('finds a table in a non-default database when searching a warehouse without listing support', () => {
        const rows = build({
            connections: [connection(motherDuckDatabases)],
            units: { my_db: loaded(motherDuckCatalog) },
            search: 'movies',
        ...
        expect(tableRows.map((row) => row.identity)).toEqual([
            {
                connectionId: CONNECTION_ID,
...
describe('buildWarehouseTreeRows with several connections', () => {
    ...
    it('keeps a listed database under its own listed name even when its catalog is keyed differently', () => {
        const rows = build({
            connections: both,
            units: {
                'connection-1/analytics': loaded(analyticsCatalog),
            ...
            expanded: [
                ...
                'connection:connection-2',
                'database:connection-2/analytics',
                'schema:connection-2/analytics/public',
            ],
        });
        ...
        expect(new Set(ids).size).toBe(ids.length);
        expect(
            ids.filter((id) => id === 'database:connection-2/raw').length,
        ).toBe(1);
        expect(ids).toContain('database:connection-2/analytics');

        const tableRow = rows.find(
            (row): row is WarehouseTreeRow & { type: 'table' } =>
                row.type === 'table' &&
                row.identity.connectionId === 'connection-2' &&
                row.identity.table === 'customers',
        );
        expect(tableRow?.identity).toEqual({
            connectionId: 'connection-2',
            database: 'analytics',
