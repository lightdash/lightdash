...
const matchesListedName = (key: string, listedName: string): boolean =>
    key === listedName || key.toLowerCase() === listedName.toLowerCase();
...
export const buildWarehouseTreeRows = ({
    ...
    const buildExtraDatabaseRows = (
        connectionId: string,
        database: string,
        schemas: WarehouseTablesCatalog[string],
    ): WarehouseTreeRow[] => {
        const nameMatches = plainMatch(database, needle);
        const id = databaseRowId(connectionId, database);
        const isExpandedRow = isRowExpanded(id, true);
        const buildChildren = (): WarehouseTreeRow[] =>
            Object.keys(schemas).flatMap((schema) =>
                buildSchemaRows({
                    connectionId,
                    database,
                    schema,
        ...
        if (isFiltering) {
            const hasChildren = (children ?? []).length > 0;
            if (!(hasChildren || nameMatches)) return [];
        }
        ...
        const header: WarehouseTreeRow = {
            ...
            listedDatabase: null,
            isExpanded: isExpandedRow,
            childCount: children !== undefined ? schemaRowCount : null,
        };
        if (!isExpandedRow) return [header];
        return [header, ...(children ?? [])];
    };
    ...
    const buildGroupRows = (
        ...
        const isPlaceholderConnection = connection.databases.length === 1;
        if (
            isPlaceholderConnection &&
            group.lazyEntry &&
            group.schemaEntries.length === 0
        ) {
            const unitState = getUnitState({
                connectionId: connection.connectionId,
                database: group.lazyEntry.name,
            });
            if (unitState.status === 'loaded') {
                const keys = Object.keys(unitState.catalog);
                const isPlaceholderMatch =
                    keys.length === 1 &&
                    matchesListedName(keys[0], group.database);
                if (keys.length > 0 && !isPlaceholderMatch) {
                    return keys.flatMap((key) =>
                        buildExtraDatabaseRows(
                            connection.connectionId,
                            key,
                            unitState.catalog[key],
                        ),
                    );
                }
            }
        }
        return buildDatabaseRows(connection, group);
    };
    ...
    return connections.flatMap((connection) => {
        ...
        const databaseRows = !isConnectionExpanded
            ...
            : connection.listingStatus === 'loaded'
              ? groups.flatMap((group) => buildGroupRows(connection, group))
              : pendingListingRows;
