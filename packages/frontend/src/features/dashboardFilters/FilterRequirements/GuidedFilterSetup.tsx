...
const RuleSummary: FC<RuleSummaryProps> = ({ rule, fieldsMap, onChange }) => {
    const setMember = rule.members.find((member) => !member.disabled);
 ...
 */
const GuidedFilterSetup: FC<Props> = ({ onDismiss, startOfWeek }) => {
    ...
    const handleChangeFilterRule = (newRule: DashboardFilterRule) => {
        ...
        if (metricIndex >= 0) {
            updateMetricDashboardFilter(updatedRule, metricIndex, false, false);
        }
    };
            ...
            >
                <Stack gap="xs" p="lg" pb="md">
                    <Group justify="space-between" align="flex-start">
                        <Group gap={6} className={classes.eyebrow}>
