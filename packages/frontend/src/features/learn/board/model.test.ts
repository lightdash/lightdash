...
import { emptyRollup, type Rollup } from '../model';
import {
    courseFor,
    ctaLabel,
    defaultRoleFor,
...
describe('groupOf', () => {
    it('regroups view:Project as foundations', () => {
        expect(groupOf(entry({ id: 'a', scope: 'view:Project' }))).toBe(
            'foundations',
        );
    });

    it('uses the scope registry group otherwise', () => {
        expect(groupOf(entry({ id: 'b', scope: 'manage:Dashboard' }))).toBe(
            ScopeGroup.CONTENT,
        );
        expect(groupOf(entry({ id: 'c', scope: 'manage:Organization' }))).toBe(
            ScopeGroup.ORGANIZATION_MANAGEMENT,
        );
    });

    it('falls back to foundations for null or unknown scopes', () => {
        expect(groupOf(entry({ id: 'd', scope: null }))).toBe('foundations');
        expect(groupOf(entry({ id: 'e', scope: 'manage:NotAThing' }))).toBe(
            'foundations',
        );
    });
});
...
describe('courseFor / isUnlocked / heldBy', () => {
    ...
    it('an admin gets everything', () => {
        expect(
            courseFor(library, getAllScopesForRole(ProjectMemberRole.ADMIN)),
        ).toHaveLength(4);
    });

    it('untagged modules are unlocked for everyone', () => {
        expect(isUnlocked(entry({ id: 'x', scope: null }), [])).toBe(true);
    });
    ...
    it('an unknown scope is unlocked for everyone, not vanished', () => {
        ...
        expect(
            SYSTEM_ROLES.every((role) =>
                isUnlocked(stale, getAllScopesForRole(role)),
            ),
        ).toBe(true);
        expect(
            courseFor([stale], getAllScopesForRole(ProjectMemberRole.VIEWER)),
        ).toHaveLength(1);
        expect(heldBy(stale)).toEqual(SYSTEM_ROLES);
    });

    it('lists the system roles that hold the module', () => {
        expect(
            heldBy(entry({ id: 'org', scope: 'manage:Organization' })),
        ).toEqual([ProjectMemberRole.ADMIN]);
        expect(heldBy(entry({ id: 'f', scope: 'view:Project' }))).toHaveLength(
            5,
        );
    });
});
...
describe('the @global marker against the real role scope sets', () => {
    ...
    it('is held by exactly the roles whose real scope set has the unmodified scope', () => {
        ...
        expect(heldBy(globalTagged)).toEqual(unmodified);
        unmodified.forEach((role) =>
            expect(isUnlocked(globalTagged, getAllScopesForRole(role))).toBe(
                true,
            ),
        );
    });
    ...
    it('a role holding only a modified variant holds the bare tag but not @global', () => {
        ...
        modifiedOnly.forEach((role) => {
            const held = getAllScopesForRole(role);
            expect(isUnlocked(globalTagged, held)).toBe(false);
            expect(isUnlocked(bareTagged, held)).toBe(true);
        });
    });
});
...
describe('scopePermits / defaultRoleFor', () => {
    it('returns the registry description for a known scope', () => {
        expect(
            scopePermits(entry({ id: 'a', scope: 'manage:Dashboard' })),
        ).toMatch(/dashboard/i);
        expect(scopePermits(entry({ id: 'b', scope: null }))).toBeNull();
    });
...
describe('railModel', () => {
    ...
    it('next up falls back to the first not-started, then to null', () => {
        const fresh = railModel(
            library,
            getAllScopesForRole(ProjectMemberRole.VIEWER),
            new Map(),
        );
        expect(fresh.nextUpId).toBe('f1');
        const finished = railModel(
            library,
            getAllScopesForRole(ProjectMemberRole.VIEWER),
            new Map([
                ['f1', rollup({ completed: true })],
        ...
        expect(railModel([], [], new Map()).nextUpId).toBeNull();
    });
});
