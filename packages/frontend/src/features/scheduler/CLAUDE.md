...
- **Filter Overrides**: Deliveries adjust the values of the resource's existing filters, they do not add new ones. Dashboards store `DashboardFilterRule[]` (applied with `applyDimensionOverrides`); charts store `Filters` (applied with `applyChartFilterOverrides`, which replaces saved rules by id and falls back to the same field). A removed override means the delivery uses the saved default. Overrides that match nothing are ANDed on, which is how threshold alerts add filters. Replacing a chart's saved rule shows rows the author filtered out, so create, update, send-now and the delivery run itself require `manage:Explore` on the project (`assertCanReplaceChartFilters`); without it the tab is read-only and the API answers 403
