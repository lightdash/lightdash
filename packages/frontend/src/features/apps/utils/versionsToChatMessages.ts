...
export const createChatMessageFallbacks = (): ChatMessageFallbacks => ({
    charts: new Map(),
    connections: new Map(),
...
export function versionsToChatMessages(
    ...
    fallbacks: ChatMessageFallbacks,
): ChatMessage[] {
    ...
    return sorted.flatMap((v) => {
        ...
        if (v.status === 'ready') {
            ...
            msgs.push({
                ...
                clarifications: [],
                appUuid: null,
                // Deliberately null: a deps chip renders off `version`, and a
