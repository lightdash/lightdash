...
export const ExploreConfigContext: React.FC = ({ children }) => {
    ...
    const activeDimensions = useMemo(
        () =>
            new Set<string>(
                dimensionSearchParam === null
                    ? []
                    : dimensionSearchParam.split(','),
            ),
    ...
    const activeMetrics = useMemo(
        () =>
            new Set<string>(
                metricSearchParam === null ? [] : metricSearchParam.split(','),
            ),
        [metricSearchParam],
    );
    ...
    const setActiveDimensions = (dimensions: Set<string>) => {
        const newParams = new URLSearchParams(searchParams);
        if (dimensions.size === 0) newParams.delete('dimensions');
        else newParams.set('dimensions', Array.from(dimensions).join(','));
        history.replace({
    ...
    const setActiveMetrics = (metrics: Set<string>) => {
        const newParams = new URLSearchParams(searchParams);
        if (metrics.size === 0) newParams.delete('metrics');
        else newParams.set('metrics', Array.from(metrics).join(','));
        history.replace({
            pathname: history.location.pathname,
            search: newParams.toString(),
        });
    ...
    const toggleActiveField = (fieldId: string, isDimension: boolean) => {
        if (isDimension) {
            ...
            if (!newDimensions.delete(fieldId)) newDimensions.add(fieldId);
            setActiveDimensions(newDimensions);
        } else {
            const newMetrics = new Set(activeMetrics);
            if (!newMetrics.delete(fieldId)) newMetrics.add(fieldId);
    ...
    const sidebarPanel: SidebarPanel =
        (searchParams.get('sidebar') || 'base') === 'explores'
            ? 'explores'
            : 'base';
    ...
    const setSortFields = useMemo(
        () => (newSortFields: SortField[]) => {
            ...
            newSortFields.filter((sf) => activeFields.has(sf.fieldId));
            const newParams = new URLSearchParams(searchParams);
            if (newSortFields.length === 0) newParams.delete('sort');
            else newParams.set('sort', JSON.stringify(newSortFields));
            history.replace({
                pathname: history.location.pathname,
                search: newParams.toString(),
            });
    ...
    const activeFilters: FilterGroup[] =
        filterSearchParam === null ? [] : JSON.parse(filterSearchParam);
    const setActiveFilters = (newActiveFilters: FilterGroup[]) => {
        const newParams = new URLSearchParams(searchParams);
        if (newActiveFilters.length === 0) newParams.delete('filters');
        else newParams.set('filters', JSON.stringify(newActiveFilters));
        history.replace({
            pathname: history.location.pathname,
            search: newParams.toString(),
        });
    ...
    const resultsRowLimit =
        limitSearchParam && !Number.isNaN(parseInt(limitSearchParam, 10))
            ? parseInt(limitSearchParam, 10)
            : 500;
    const setResultsRowLimit = (rowLimit: string | undefined) => {
        const newParams = new URLSearchParams(searchParams);
        if (rowLimit === undefined) newParams.delete('limit');
        else newParams.set('limit', `${rowLimit}`);
        history.replace({
            pathname: history.location.pathname,
            search: newParams.toString(),
        });
