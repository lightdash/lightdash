...
export const applyStackedBarBorderRadius = (
    ...
    Object.entries(seriesByStack).forEach(([, stackSeries]) => {
        const reversedSeries = [...stackSeries].reverse();

        for (let dataIndex = 0; dataIndex < rows.length; dataIndex++) {
            // Find the topmost series with data at this index
            const topSeries = reversedSeries.find((serie) => {
    ...
    if (isFlipAxes) {
        ...
        return {
            ...
            yAxis: axes.yAxis.map((axis, idx) =>
                idx === 0 ? ({ ...axis, data: categoryData } as typeof axis) : axis,
            ),
        };
    } else {
