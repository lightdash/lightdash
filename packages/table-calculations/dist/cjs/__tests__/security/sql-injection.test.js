...
describe('SQL Injection Prevention', () => {
    describe('Field Reference Injection Attempts', () => {
        it('should reject field names with SQL injection attempts', async () => {
            ...
            for (const expr of maliciousFields) {
                try {
                    const ast = await effect_1.Effect.runPromise((0, parser_1.parse)(expr));
                    const sql = await effect_1.Effect.runPromise((0, generators_1.generateSQL)(ast, {
                        dialect: 'postgres',
    ...
    describe('String Literal Injection Attempts', () => {
        ...
        it('should handle single quote escaping correctly', async () => {
            ...
            for (const testCase of testCases) {
                const ast = await effect_1.Effect.runPromise((0, parser_1.parse)(testCase.input));
                const sql = await effect_1.Effect.runPromise((0, generators_1.generateSQL)(ast, {
                    dialect: 'postgres',
                    fieldResolver: (field) => effect_1.Effect.succeed(field),
                }));
    ...
    describe('Function Name Injection Attempts', () => {
        it('should reject malicious function names', async () => {
            ...
            for (const funcName of maliciousFunctions) {
                ...
                const result = await effect_1.Effect.runPromiseExit((0, parser_1.parse)(expr));
        ...
        it('should prevent injection through nested expressions', async () => {
            ...
            for (const expr of maliciousExpressions) {
                ...
                if (result._tag === 'Success') {
                    ...
                    const sql = await effect_1.Effect.runPromise((0, generators_1.generateSQL)(result.value, {
                        dialect: 'postgres',
                        fieldResolver: (field) => effect_1.Effect.succeed(field),
                    }));
                    expect(sql).not.toContain('DROP');
    ...
    describe('Field Validation Security', () => {
        it('should prevent access to non-whitelisted fields', async () => {
            const ast = await effect_1.Effect.runPromise((0, parser_1.parse)('\${users.password}'));
            const result = await effect_1.Effect.runPromiseExit((0, validator_1.validate)(ast, {
        ...
        it('should only allow explicitly whitelisted fields', async () => {
            ...
            for (const fieldName of maliciousFields) {
                const ast = await effect_1.Effect.runPromise((0, parser_1.parse)(`\${${fieldName}}`));
                const result = await effect_1.Effect.runPromiseExit((0, validator_1.validate)(ast, { availableFields: allowedFields }));
                expect(result._tag).toBe('Failure');
                if (result._tag === 'Failure' && result.cause._tag === 'Fail') {
                    expect(result.cause.error._tag).toBe('UnknownFieldError');
                }
    ...
    describe('Edge Cases and Malformed Input', () => {
        ...
        it('should reject completely malformed SQL-like input', async () => {
            ...
            for (const input of malformedInputs) {
                const result = await effect_1.Effect.runPromiseExit((0, parser_1.parse)(input));
