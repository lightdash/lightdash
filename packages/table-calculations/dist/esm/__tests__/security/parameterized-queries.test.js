...
import { generateParameterizedSQL } from '../../generators';
...
describe('Parameterized Query Security', () => {
    describe('Field Reference Parameterization', () => {
        it('should use parameter placeholders for field references', async () => {
            ...
            const program = pipe(parse(`[${maliciousFieldName}]`), Effect.flatMap((ast) => generateParameterizedSQL(ast, {
                dialect: 'postgres',
        ...
        it('should handle complex expressions with multiple field references', async () => {
            ...
            const program = pipe(parse(`[${maliciousFields[0]}] + [${maliciousFields[1]}]`), Effect.flatMap((ast) => generateParameterizedSQL(ast, {
                dialect: 'postgres',
                parameterizedFieldResolver,
            })));
            const result = await Effect.runPromise(program);
    ...
    describe('Function Calls with Parameterized Fields', () => {
        it('should parameterize field references within function calls', async () => {
            ...
            const program = pipe(parse(`sum([${maliciousFieldName}])`), Effect.flatMap((ast) => generateParameterizedSQL(ast, {
                dialect: 'postgres',
                parameterizedFieldResolver,
            })));
            const result = await Effect.runPromise(program);
    ...
    describe('Conditional Expressions with Parameterization', () => {
        it('should parameterize field references in conditional expressions', async () => {
            ...
            const program = pipe(parse(`if([${maliciousFields[0]}] = "active", [${maliciousFields[1]}], [${maliciousFields[2]}])`), Effect.flatMap((ast) => generateParameterizedSQL(ast, {
                dialect: 'postgres',
                parameterizedFieldResolver,
            })));
            const result = await Effect.runPromise(program);
    ...
    describe('Backwards Compatibility', () => {
        it('should fall back to standard field resolution when parameterized is not available', async () => {
            const standardFieldResolver = (fieldName) => {
                return Effect.succeed(`safe_${fieldName}`);
            };
            const program = pipe(parse('\${user_id}'), Effect.flatMap((ast) => generateParameterizedSQL(ast, {
                dialect: 'postgres',
    ...
    describe('Parameter Types', () => {
        it('should support different parameter types for different databases', async () => {
            ...
            for (const testCase of testCases) {
                ...
                const program = pipe(parse('\${revenue}'), Effect.flatMap((ast) => generateParameterizedSQL(ast, {
    ...
    describe('Validation Integration', () => {
        it('should validate field names before parameterization', async () => {
            ...
            const program = pipe(parse(`\${${maliciousField}}`), Effect.flatMap((ast) => validate(ast, { availableFields: ['revenue', 'cost'] })), Effect.flatMap((ast) => generateParameterizedSQL(ast, {
                dialect: 'postgres',
                parameterizedFieldResolver,
            })));
