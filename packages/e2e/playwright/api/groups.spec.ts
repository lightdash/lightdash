...
test.describe('Groups API', () => {
    ...
    test('should return a group to admin', async ({ request }) => {
        const response = await request.get(`api/v1/groups/${SEED_GROUP.groupUuid}`);
        expect(response.status()).toBe(200);
    });

    test('should forbid group outside the organization', async ({ request }) => {
    ...
    test('should return a list of groups in organization', async ({ request }) => {
    ...
    test('should not return groups to another organization', async ({ request }) => {
        await anotherLogin(request);
        const response = await request.get('api/v1/org/groups');
        expect(response.status()).toBe(200);
    ...
    test('should delete group from organization', async ({ request }) => {
        ...
        const deleteResponse = await request.delete(`api/v1/groups/${createBody.results.uuid}`);
    ...
    test('should update group name', async ({ request }) => {
        ...
        const updateResponse = await request.patch(`api/v1/groups/${createBody.results.uuid}`, {
            data: {
                name: newGroupName,
            },
        });
    ...
    test('should get group members', async ({ request }) => {
        const response = await request.get(`api/v1/groups/${SEED_GROUP.groupUuid}/members`);
        expect(response.status()).toBe(200);
    });

    test('should successfully update group name and members', async ({ request }) => {
        const createResponse = await request.post('api/v1/org/groups', {
            data: {
                name: `Test group${new Date().getTime()}`,
            },
        ...
        expect(updateBody.results.members[0].userUuid).toBe(SEED_ORG_1_ADMIN.user_uuid);

        const clearResponse = await request.patch(`api/v1/groups/${createBody.results.uuid}`, {
            data: {
                members: [],
            },
        });
    ...
    test('should not add a user from another org to a group', async ({ request }) => {
        const createResponse = await request.post('api/v1/org/groups', {
            data: {
    ...
    test.describe('Group Project access API', () => {
        ...
        test('should not add a group access to a project for another organization', async ({ request }) => {
            await anotherLogin(request);
            const response = await request.post(
                `api/v1/groups/${SEED_GROUP.groupUuid}/projects/${SEED_PROJECT.project_uuid}`,
                {
        ...
        test('should update a group access to a project', async ({ request }) => {
        ...
        test('should not update a group access to a project for another organization', async ({ request }) => {
            await anotherLogin(request);
            const response = await request.patch(
                `api/v1/groups/${SEED_GROUP.groupUuid}/projects/${SEED_PROJECT.project_uuid}`,
                {
        ...
        test('should remove a group access from a project', async ({ request }) => {
        ...
        test('should not remove a group access from a project for another organization', async ({ request }) => {
            await anotherLogin(request);
            const response = await request.delete(
                `api/v1/groups/${SEED_GROUP.groupUuid}/projects/${SEED_PROJECT.project_uuid}`,
            );
