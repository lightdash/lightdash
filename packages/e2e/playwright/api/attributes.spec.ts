...
test.describe('Attributes API', () => {
    ...
    test('creates an attribute with users and groups', async ({ request }) => {
        ...
        const attribute = getBody.results.find((a: { uuid: string }) => a.uuid === uuid);
    ...
    test('creates an empty attribute and updates with users and groups', async ({ request }) => {
        const name = `example_attribute${Math.random()}`;
        ...
        const { uuid } = body.results;
        const putResponse = await request.put(`/api/v1/org/attributes/${uuid}`, {
            data: {
                name: newName,
                users: [
                    {
        ...
        expect(putBody.results.users[0].userUuid).toBe(SEED_ORG_1_ADMIN.user_uuid);
