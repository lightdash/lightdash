...
test.describe('Lightdash pinning endpoints', () => {
    ...
    test('Should pin/unpin chart', async ({ request }) => {
        ...
        // Pin Chart
        const pinResponse = await request.patch(
            `${apiUrl}/saved/${savedChart.uuid}/pinning`,
        );
        const pinBody = await pinResponse.json();

        // Verify chart is pinned
        const res1Response = await request.get(
            `${apiUrl}/projects/${projectUuid}/charts`,
        );
        const res1Body = await res1Response.json();
        expect(res1Body.results[0].pinnedListUuid).toBe(
            pinBody.results.pinnedListUuid,
        );
        ...
        const res2Response = await request.get(
            `${apiUrl}/projects/${projectUuid}/charts`,
        );
