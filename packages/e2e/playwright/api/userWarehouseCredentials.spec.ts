...
test.describe('User Warehouse Credentials API', () => {
    test.beforeEach(async ({ request }) => {
        ...
        const deletePromises = getBody.results.map(
            ({ uuid }: { uuid: string }) =>
    ...
    test('should list/create/update/delete warehouse credentials', async ({
        request,
    }) => {
        // get all (empty)
        const getEmptyResponse = await request.get(
            'api/v1/user/warehouseCredentials',
        );
        ...
        expect(createBody.results.credentials.type).toBe(
            CREATE_CREDENTIALS_MOCK.credentials.type,
        );
        expect(createBody.results.credentials.user).toBe(
        ...
        expect(getBody.results[0].credentials.type).toBe(
            CREATE_CREDENTIALS_MOCK.credentials.type,
        );
        expect(getBody.results[0].credentials.user).toBe(
            CREATE_CREDENTIALS_MOCK.credentials.user,
        );
        ...
        const updateResponse = await request.patch(
            `api/v1/user/warehouseCredentials/${createBody.results.uuid}`,
            {
        ...
        expect(updateBody.results.credentials.type).toBe(
            UPDATE_CREDENTIALS_MOCK.credentials.type,
        );
        expect(updateBody.results.credentials.user).toBe(
    ...
    test('should get/update project user warehouse credentials preference', async ({
        request,
    }) => {
        ...
        const getFallbackResponse = await request.get(
            `api/v1/projects/${SEED_PROJECT.project_uuid}/user-credentials`,
        );
        ...
        const createSecondResponse = await request.post(
            'api/v1/user/warehouseCredentials',
            {
                data: CREATE_CREDENTIALS_MOCK,
            },
        ...
        await request.patch(
            `api/v1/projects/${SEED_PROJECT.project_uuid}/user-credentials/${createSecondBody.results.uuid}`,
        );

        // get preference result
        const getPreferenceResponse = await request.get(
            `api/v1/projects/${SEED_PROJECT.project_uuid}/user-credentials`,
        );
        expect(getPreferenceResponse.status()).toBe(200);
        const getPreferenceBody = await getPreferenceResponse.json();
        expect(getPreferenceBody.results.uuid).toBe(
            createSecondBody.results.uuid,
