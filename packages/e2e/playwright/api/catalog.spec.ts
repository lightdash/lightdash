...
test.describe('Lightdash catalog all tables and fields', () => {
    ...
    test('Should list all tables', async ({ request }) => {
        ...
        const userTable = body.results.find(
            (table: AnyType) => table.name === 'users',
        );
...
test.describe('Lightdash catalog search', () => {
    ...
    test('Should search for customer tables and fields', async ({
        request,
    }) => {
        const projectUuid = SEED_PROJECT.project_uuid;
        const response = await request.get(
        ...
        const table = body.results.find(
            (t: AnyType) => t.name === 'customers' && t.type === 'table',
        );
        expect(table).toHaveProperty('name', 'customers');

        const field = body.results.find(
            (f: AnyType) =>
                f.name === 'customer_id' && f.tableLabel === 'Users',
        );
        expect(field).toHaveProperty('name', 'customer_id');
    });

    test('Should search for a dimension (payment_method)', async ({
        request,
    }) => {
        const projectUuid = SEED_PROJECT.project_uuid;
        const response = await request.get(
    ...
    test('Should search for a metric (total_revenue) sorted by chartUsage', async ({
        request,
    }) => {
        const projectUuid = SEED_PROJECT.project_uuid;
        const response = await request.get(
    ...
    test('Should search with partial word (cust)', async ({ request }) => {
        ...
        // Check for a table
        const matchingTable = body.results.find(
            (t: AnyType) => t.name === 'customers',
        );
        expect(matchingTable).toHaveProperty('name', 'customers');
    });

    test('Should search with multiple words (order date)', async ({
        request,
    }) => {
        const projectUuid = SEED_PROJECT.project_uuid;
        const response = await request.get(
        ...
        const matchingField = body.results.find(
            (f: AnyType) =>
                f.name === 'date_of_first_order' && f.type === 'field',
        );
    ...
    test('Should filter fields with required attributes (age)', async ({
        request,
    }) => {
        const projectUuid = SEED_PROJECT.project_uuid;
        const response = await request.get(
    ...
    test('Should filter table with required attributes (memberships)', async ({
        request,
    }) => {
        const projectUuid = SEED_PROJECT.project_uuid;
        const response = await request.get(
    ...
    test('Should filter field in table without attribute access (plan)', async ({
        request,
    }) => {
        const projectUuid = SEED_PROJECT.project_uuid;
        const response = await request.get(
...
test.describe('Lightdash analytics', () => {
    ...
    test('Should get analytics for customers table', async ({ request }) => {
        ...
        const chart = body.results.charts.find(
            (c: AnyType) =>
                c.name === 'How many users were created each month ?',
        );
...
test.describe('Lightdash catalog search with yaml tags', () => {
    ...
    test('Should be able to find PII fields when no yaml tags are provided', async ({
        request,
    }) => {
        const projectUuid = SEED_PROJECT.project_uuid;
        const response = await request.get(
    ...
    test('Should not be able to find any PII fields which does not have the ai tag', async ({
        request,
    }) => {
        const projectUuid = SEED_PROJECT.project_uuid;
        const response = await request.get(
    ...
    test('Should be able to find tables that have the ai tag when yaml tags are provided', async ({
        request,
    }) => {
        const projectUuid = SEED_PROJECT.project_uuid;
        const response = await request.get(
    ...
    test('Should be able to find tagged tables when no yaml tags are provided', async ({
        request,
    }) => {
        const projectUuid = SEED_PROJECT.project_uuid;
        const response = await request.get(
    ...
    test('Should not be able to find tables that do not have the ai tag when yaml tags are provided', async ({
        request,
    }) => {
        const projectUuid = SEED_PROJECT.project_uuid;
        const response = await request.get(
    ...
    test('Should be able to find tagged fields from untagged table', async ({
        request,
    }) => {
        const projectUuid = SEED_PROJECT.project_uuid;
        const response = await request.get(
    ...
    test('should be able to find fields if table is tagged but field is not', async ({
        request,
    }) => {
        const projectUuid = SEED_PROJECT.project_uuid;
        const response = await request.get(
    ...
    test('should be able to find table if table is untagged but field is tagged', async ({
        request,
    }) => {
        const projectUuid = SEED_PROJECT.project_uuid;
        const response = await request.get(
