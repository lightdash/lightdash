...
test.describe('Search API - Enhanced Dashboard and Chart Search', () => {
    ...
    test.describe('Dashboard search with chart content', () => {
        test('Should find dashboards by searching for chart content', async ({ request }) => {
        ...
        test('Should include charts from both direct and tile relationships in dashboard results', async ({ request }) => {
            const response = await request.get(
                `${apiUrl}/projects/${SEED_PROJECT.project_uuid}/search/${searchKeyword}`,
            );
            expect(response.status()).toBe(200);
            ...
            expect(jaffleChartNames).toContain("What's our total revenue to date?");

            const dashboardWithCharts = dashboards.find(
                (d: SearchResult) => d.name === 'Dashboard with dashboard charts',
            );
        ...
        test('Should rank dashboards by relevance including chart content', async ({ request }) => {
            const response = await request.get(
                `${apiUrl}/projects/${SEED_PROJECT.project_uuid}/search/${searchKeyword}`,
            );
            expect(response.status()).toBe(200);
    ...
    test.describe('Chart metadata in search results', () => {
        test('Should handle SQL charts with null metadata gracefully', async ({ request }) => {
            const response = await request.get(
    ...
    test.describe('Search edge cases', () => {
        test('Should return empty results for non-matching searches', async ({ request }) => {
            const response = await request.get(
