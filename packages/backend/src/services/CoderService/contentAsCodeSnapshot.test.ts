...
describe('buildContentAsCodeSnapshot', () => {
    ...
    // The instance materialises defaults a hand-authored file omits, and a
    // downloaded file spells the same absence as null. All three must hash
    // alike or fast_forward is unreachable.
    it('hashes absent, null and empty the same way', () => {
        const absent = buildContentAsCodeSnapshot(chartAsCode());
        // A downloaded file spells an unset list as null
        const asNull = buildContentAsCodeSnapshot(
            chartAsCode({
                metricQuery: {
                    ...chartAsCode().metricQuery,
                    customDimensions: null,
                },
            } as unknown as Partial<ChartAsCode>),
        );
        const asEmptyArray = buildContentAsCodeSnapshot(
            chartAsCode({
                metricQuery: {
                    ...chartAsCode().metricQuery,
                    customDimensions: [],
        ...
        expect(asNull.snapshotHash).toEqual(absent.snapshotHash);
        expect(asEmptyArray.snapshotHash).toEqual(absent.snapshotHash);
    });

    it('hashes an empty object the same as an absent one', () => {
        const absent = buildContentAsCodeSnapshot(
            chartAsCode({ tableConfig: { columnOrder: [] } }),
        );
        const withEmptyParameters = buildContentAsCodeSnapshot(
            chartAsCode({
                tableConfig: { columnOrder: [] },
                parameters: {},
            } as Partial<ChartAsCode>),
        );

        expect(withEmptyParameters.snapshotHash).toEqual(absent.snapshotHash);
    });

    it('still separates an empty value from a populated one', () => {
        const empty = buildContentAsCodeSnapshot(chartAsCode());
        const populated = buildContentAsCodeSnapshot(
            chartAsCode({
                metricQuery: {
                    ...chartAsCode().metricQuery,
                    customDimensions: [
        ...
        expect(populated.snapshotHash).not.toEqual(empty.snapshotHash);
    });
});
