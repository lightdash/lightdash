...
export class ProjectService extends BaseService {
    ...
    */
    ...
    async runViewChartQuery({
        ...
        const auditedAbility = this.createAuditedAbility(account);
        if (
            auditedAbility.cannot('view', subject('SavedChart', spaceCtx)) ||
            auditedAbility.cannot(
                'view',
                subject('Project', {
                    organizationUuid,
    ...
    async getJobStatus(jobUuid: string, user: SessionUser): Promise<Job> {
        ...
        } else if (auditedAbility.cannot('view', subject('Job', job))) {
            throw new NotFoundError(`Cannot find job`);
        }
    ...
    async getSpaces(
        ...
        return spaces
            .filter((space) => {
                ...
                return ctx && auditedAbility.can('view', subject('Space', ctx));
            })
            .map((spaceSummary) => {
