...
export abstract class BaseService {
    ...
    protected createAuditedAbility(
        ...
        this.logger.debug('Creating audited ability', {
            accountType:
                'authentication' in accountOrUser
                    ? accountOrUser.authentication.type
                    : 'session-user',
        });

        // Determine if this is an Account or a SessionUser
        if ('authentication' in accountOrUser) {
            // Account type
            const account = accountOrUser as Account;
            return new CaslAuditWrapper(account.user.ability, account, {
                callStack,
        ...
        const user = accountOrUser as SessionUser;
        return new CaslAuditWrapper(user.ability, user, {
            callStack,
            auditLogger: logAuditEvent,
        });
    }
