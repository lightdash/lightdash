...
export const createRenameFactory = ({
    ...
    if (isPrefix) {
        ...
        replaceFieldReference = (str: string) => str; // no changes
...
export const getNameChanges = ({
    ...
    const newTable = to.replace(`_${fromWithoutTable}`, '');

    // TODO handle case when both table and field are different
    return {
        from: table,
        to: newTable,
        fromReference: table,
        toReference: newTable,
...
export const renameSavedChart = (
    ...
    if (!containsModelName(chart)) {
        console.debug(
            `No references to "${nameChanges.from}" in chart "${chart.name}"`,
        );
