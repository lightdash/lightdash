    ...
    private async createZipDownloadUrl({
        ...
        try {
                ...
                files.map(async (file) => {
                    ...
                    try {
                        ...
                        if (!response.ok || !response.body) {
                            Logger.warn(
                                `Failed to fetch export file "${file.filename}": HTTP ${response.status} ${response.statusText}`,
                            );
                            return null;
                        }
                        ...
                        Logger.error(
                            `Failed to fetch export file "${file.filename}": ${getErrorMessage(e)}`,
                            {
            ...
            const validStreams = streams.filter(
                (stream): stream is { filename: string; stream: Readable } =>
                    stream !== null,
            );
            ...
            validStreams.forEach((file) => {
                archive.append(file.stream, {
                    name: `${deduplicateName(file.filename)}.${fileType}`,
                });
            });
