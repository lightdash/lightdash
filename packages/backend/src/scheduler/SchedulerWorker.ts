...
export class SchedulerWorker extends SchedulerTask {
    ...
    protected getFullTaskList(): TypedTaskList {
        return {
            ...
            [SCHEDULER_TASKS.ONBOARDING_SEMANTIC]: async (payload, helpers) => {
                await SchedulerClient.processJob(
            ...
            [SCHEDULER_TASKS.ONBOARDING_DASHBOARD]: async (
                payload,
                helpers,
            ) => {
                await SchedulerClient.processJob(
                    ...
                    payload,
                    async () => {
                        await this.onboardingDashboard(
                            helpers.job.id,
                            helpers.job.run_at,
                            payload,
                        );
