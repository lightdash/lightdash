...
describe('BufferedEventStreamWriter', () => {
    ...
    it('buffers rows without writing until flushed', async () => {
        const writer = createWriter(metrics);
        const testRow = row();
        writer.push('query_executed', testRow);

        expect(s3Mocks.putObject).not.toHaveBeenCalled();
        expect(metrics.incrementUsageEventsPushed).toHaveBeenCalledTimes(1);
        ...
        expect(metrics.incrementUsageEventsFlushed).toHaveBeenCalledWith(1);

        await writer.close();
    });

    it('flushes automatically when the batch size is reached', async () => {
        const writer = createWriter(metrics, { flushBatchSize: 2 });
        ...
        expect(metrics.incrementUsageEventsFlushed).toHaveBeenCalledWith(2);

        await writer.close();
    });

    it('flushes on the background interval', async () => {
        const writer = createWriter(metrics, { flushIntervalMs: 20 });
        writer.push('query_executed', row());
    ...
    it('drops new events and counts them when the buffer is full', async () => {
        const writer = createWriter(metrics, { bufferMaxSize: 2 });
        writer.push('query_executed', row());
        writer.push('query_executed', row());
        writer.push('query_executed', row());

        expect(metrics.incrementUsageEventsPushed).toHaveBeenCalledTimes(2);
        expect(metrics.incrementUsageEventsDropped).toHaveBeenCalledTimes(1);

        await writer.flush();
        const putArgs = s3Mocks.putObject.mock.calls[0][0];
        expect(decodeBody(putArgs.Body).trimEnd().split('\n')).toHaveLength(2);

        await writer.close();
    });

    it('retries failed PUTs before succeeding', async () => {
        s3Mocks.putObject
        ...
        expect(s3Mocks.putObject).toHaveBeenCalledTimes(3);
        expect(metrics.incrementUsageEventsFlushed).toHaveBeenCalledWith(1);
        expect(metrics.incrementUsageEventsPutFailure).not.toHaveBeenCalled();

        await writer.close();
    });

    it('drops the batch, warns and counts the failure after exhausting retries', async () => {
        s3Mocks.putObject.mockRejectedValue(new Error('network error'));
        const writer = createWriter(metrics);
        writer.push('query_executed', row());
        ...
        expect(s3Mocks.putObject).toHaveBeenCalledTimes(3);
        expect(metrics.incrementUsageEventsFlushed).not.toHaveBeenCalled();
        expect(metrics.incrementUsageEventsPutFailure).toHaveBeenCalledTimes(1);
        expect(Logger.warn).toHaveBeenCalledWith(
            expect.stringContaining('Dropping 1 usage events'),
        );
    ...
    it('close flushes remaining rows and drops any further pushes', async () => {
        const writer = createWriter(metrics);
        writer.push('query_executed', row());
...
describe('createEventStreamWriter', () => {
    it('returns null when usage events are disabled', () => {
        const writer = createEventStreamWriter(
            lightdashConfigMock,
            createMetricsMock() as unknown as PrometheusMetrics,
        );
        expect(writer).toBeNull();
    });

    it('returns a writer when usage events are enabled with S3 config', async () => {
        const writer = createEventStreamWriter(
            {
                ...lightdashConfigMock,
                usageEvents: {
        ...
        expect(writer).toBeInstanceOf(BufferedEventStreamWriter);
        await writer?.close();
    });
});
