...
const systemStreamMetrics: Record<string, SystemMetricDef[]> = {
    // query_events has one row per query (query.completed), so COUNT = queries
    query_events: [
        {
            name: 'total_queries',
    ...
    // ai_usage has one row per AI model call (ai.usage), so COUNT = calls
    ai_usage: [
        {
            name: 'total_ai_calls',
        ...
        {
            // 'total_tokens' would collide with the dimension of the same name
