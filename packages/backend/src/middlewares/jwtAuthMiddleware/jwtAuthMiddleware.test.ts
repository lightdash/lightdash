...
const { decodeJwt } = require('../auth/lightdashJwt');
...
describe('Embed Auth Middleware', () => {
    ...
    describe('Successful authentication scenarios', () => {
        it('should authenticate with embed token from lightdash-embed-token header', async () => {
            ...
            decodeJwt.mockReturnValue(mockEmbedJwt);
    ...
    describe('Path parsing scenarios', () => {
        it.each([
            '/api/v1/embed/project-123/dashboard',
            '/api/v1/projects/project-456/chart',
        ])('should extract project UUID from embed path: %s', async (path) => {
            ...
            // Calculate expected project UUID based on middleware logic
            const pathParts = path.split('/');
            const embedIndex = pathParts.findIndex((part) => part === 'embed');
            const expectedProjectUuid = pathParts[embedIndex + 1];

            expect(mockRequest.project).toEqual({
                projectUuid: expectedProjectUuid,
            });
        });
