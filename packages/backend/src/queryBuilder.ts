...
const renderStringFilterSql = (dimensionSql: string, filter: StringFilter): string => {
...
const renderNumberFilterSql = (dimensionSql: string, filter: NumberFilter): string => {
    const filterType = filter.operator
    switch (filter.operator) {
        case "equals":
    ...
    switch (filterGroup.type) {
        case "string": {
            if (dimension?.type === 'string')
                return filterGroup.filters.map(filter => renderStringFilterSql(dimension.compiledSql, filter)).join(`\n   ${operator} `)
        ...
        case "number":
            if (dimension?.type === 'number')
                return filterGroup.filters.map(filter => renderNumberFilterSql(dimension.compiledSql, filter)).join(`\n   ${operator} `)
