...
export class ValidationModel {
    ...
    async getPaginated(
        ...
        const rows: NormalizedRow[] = await this.database
            ...
            .modify((qb) => {
                if (searchQuery) {
                    void qb.where((inner) => {
                        void inner
                            .where('resource_name', 'ILIKE', `%${searchQuery}%`)
        ...
        const data: ValidationResponse[] = rows.map((row) => {
            if (row.source === ValidationSourceType.Chart) {
                const result: ValidationErrorChartResponse = {
            ...
            if (row.source === ValidationSourceType.Dashboard) {
                ...
                const result: ValidationErrorDashboardResponse = {
                    validationId: row.validation_id,
                    createdAt: row.created_at,
                    projectUuid: row.project_uuid,
                    error: row.error,
            ...
            const result: ValidationErrorTableResponse = {
                validationId: row.validation_id,
                createdAt: row.created_at,
                projectUuid: row.project_uuid,
                error: row.error,
