...
export class SlackChannelCacheModel {
    ...
    async startSync(organizationId: number): Promise<boolean> {
        ...
        const existingStatus = await this.getSyncStatus(organizationId);

        if (!existingStatus) {
            return false; // No Slack installation exists
        }

        if (
            existingStatus.channels_sync_status === SchedulerJobStatus.STARTED
        ) {
            // Check if it's a stale lock (e.g., process crashed)
            // Consider it stale if it's been running for more than 10 minutes
            const staleThreshold = 10 * 60 * 1000; // 10 minutes
            const lockAge = existingStatus.channels_last_sync_at
                ? now.getTime() - existingStatus.channels_last_sync_at.getTime()
                : 0;

            if (lockAge < staleThreshold) {
                return false; // Sync is already in progress
            }
            // Lock is stale, proceed to update it
        }
