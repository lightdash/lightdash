...
export class DbtChildProcess {
    ...
    private _storeErrorMessage(payload: {message?: string, levelname?: string}) {
        if (payload.message && (payload.levelname === 'ERROR' || payload.levelname === 'WARNING')) {
            this.errorLogs = [
                payload['message'],
                ...this.errorLogs.slice(0, 5),
            ];
        }
    }

    private _logMessageShowsServerReady(payload: {message?: string}) {
        if (payload.message) {
