...
const dbtUnreachableErrorMessage = 'Cannot connect to dbt. Please check your dbt configuration.';
...
export class DbtRpcClient {
    ...
    private async _waitForDbtServerReady (): Promise<boolean> {
        ...
        const poll = (interval: number) => (
            async (resolve: (value: boolean) => void, reject: (reason: any) => void): Promise<any> => {
                ...
                // TODO: what was this protecting against?
    ...
    public async runQuery (query: string): Promise<{ [col: string]: any }> {
        const params = {
