...
describe('account', () => {
    describe('fromJwt', () => {
        ...
        it.each(['legacy', 'roles'] as const)(
            'resolves structured interactivity in %s mode without changing hidden filters',
            (permissionsMode) => {
                for (const granted of [false, true]) {
                    ...
                    const allowed = granted || permissionsMode === 'legacy';
                    expect(Boolean(result.access.filtering?.enabled)).toBe(
                        allowed,
                    );
                    expect(result.access.filtering?.canAddFilters).toBe(
                        allowed,
                    );
                    expect(result.access.parameters?.enabled).toBe(allowed);
                    expect(result.access.filtering?.hidden).toBe(true);
                    expect(result.access.filtering?.allowedFilters).toEqual([
                        'department',
                    ]);
                    expect(result.authentication.data).toEqual(token);
                }
            },
        );

        it('rejects role mode with no resolved actor instead of using legacy flags', () => {
            expect(() =>
                fromJwt({
                    decodedToken: {
                        ...mockDecodedToken,
