...
describe('pre-aggregate materialization compatibility payload', () => {
    ...
    test('keeps compatibility stable as relative windows advance, but executes fresh boundaries', () => {
        ...
        expect(next.composer.getSql({ columnLimit: 100 })).not.toBe(
            first.composer.getSql({ columnLimit: 100 }),
        );
    });
    ...
    test('password rotation, presentation changes, and unused actor attributes do not rebuild', () => {
        ...
        expect(actor.compatibilityHash).toBe(original.compatibilityHash);
        expect(actor.pinnedContextHash).not.toBe(original.pinnedContextHash);
    });

    test('used intrinsic attributes remain part of compatibility through generated SQL', () => {
        const first = prepare({
    ...
    test('uses physical numeric types for average components and retains serving-only formula reuse', () => {
        ...
        expect(prepare({ derivedSql: '${total} * 100' }).prepared).toEqual(
