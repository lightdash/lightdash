...
Wire the standard data-point action: when \`useVizContext().pointMenu?.enabled\` is true, a mark click calls \`pointMenu.open({ x: event.clientX, y: event.clientY, row: datum.sourceRow, metric: '<field name>' })\` and the viz renders NO menu of its own — Lightdash renders its native context menu (copy value, view underlying data, drill, dashboard cross-filtering) over the viz and owns all subsequent UI. Only when \`pointMenu?.enabled\` is not true (older hosts, or an SDK that predates \`pointMenu\` — always use the optional chain so this degrades instead of crashing), fall back to a minimal in-viz action menu offering "View underlying data" via \`underlyingData.open({ row, metric })\` when \`underlyingData.enabled\`, and a drill action via \`drillDown.open({ row, metric })\` when \`drillDown.enabled\`. Keep the untransformed source row on every interactive datum; skip the action on marks that aggregate multiple rows. The \`reusable-visualization\` skill has the full contract.
