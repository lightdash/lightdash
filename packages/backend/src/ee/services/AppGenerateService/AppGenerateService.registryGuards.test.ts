...
describe('read-only invariant for registry-managed apps', () => {
    ...
    it('restoreApp restores a registry app with data apps off, library on', async () => {
    ...
    it('restoreApp still requires data apps for a non-registry app', async () => {
        const { service } = buildService({
            appModel: {
                getAppIncludingDeleted: vi.fn().mockResolvedValue(baseApp),
                restore: vi.fn(),
            },
            featureFlags: { [FeatureFlags.EnableDataApps]: false },
        });
