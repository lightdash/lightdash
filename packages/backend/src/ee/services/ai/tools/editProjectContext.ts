...
export const getEditProjectContext = ({ editProjectContext }: Dependencies) =>
    tool({
        ...
        execute: async ({ op, id, kind, content, terms, objects }) => {
            try {
                // The conversational tool contract has no title/apply; those
                // are generated by the review judge only. Null means "not
                // provided": an update preserves the entry's existing values.
