...
describe('getRunQuery custom chart types', () => {
    ...
    it('runs the query and persists the uuid-enriched saved-chart shape', async () => {
            ...
            expect.objectContaining({
                vizConfig: {
                    ...
                    config: {
                        ...
                        chartConfig: {
                            dataAppVizUuid:
                                '4c25c1d5-cbc9-4d76-b58e-b1c9ee399fd9',
                            fieldMapping: { x: 'a_dim1', y: 'a_met1' },
    ...
    it('omits optionValues when the agent sets no options', async () => {
        const { createOrUpdateArtifact } = await executeCustom({
            chartConfig: { ...customChartConfig, options: null },
        });

        const { chartConfig: persistedChartConfig } =
            createOrUpdateArtifact.mock.calls[0][0].vizConfig.config;
        expect(persistedChartConfig).toEqual({
            dataAppVizUuid: '4c25c1d5-cbc9-4d76-b58e-b1c9ee399fd9',
            fieldMapping: { x: 'a_dim1', y: 'a_met1' },
        });
    });
