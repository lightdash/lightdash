    ...
    tool({
        ...
        execute: async (toolArgs, { experimental_context: context }) => {
            try {
                ...
                const tableCalculations = convertAiTableCalcsSchemaToTableCalcs(
                    queryTool.tableCalculations,
                );
                ...
                const fieldsMap = getItemMap(
                    explore,
                    populatedCustomMetrics,
                    tableCalculations,
                );
                const chartConfig = getRunQueryChartConfig({
                    queryTool: expandedQueryTool,
                    metricQuery,
                    fieldsMap,
                });
                const title = queryTool.title || 'Generated by Lightdash';
