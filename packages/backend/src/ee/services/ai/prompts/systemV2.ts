...
export const getSystemPromptV2 = (args: {
    ...
    const grepFieldsSection = enableGrepFields
        ? [
              ...
              '- A description or hint ending in "...(truncated)" is incomplete, and the hidden part may change how the field must be used (caveats, known data anomalies, attribution rules). Never interpret or present results from such a field without first reading its full hint via getMetadata.',
