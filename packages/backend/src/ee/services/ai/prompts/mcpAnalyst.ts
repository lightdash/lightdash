...
1. **Find explores first**: Use \`find_explores\` with a natural language search query to discover relevant data models and any matching verified answers
   ...
   - Review \`topMatchingDimensions\` and \`topMatchingMetrics\` to understand which explores match
...
3. **Get exact fields**: Use \`get_fields\` once you know exact field ids that are likely to be used and need full field details before final query construction
