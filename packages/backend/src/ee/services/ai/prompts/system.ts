...
export const getSystemPrompt = (args: {
    ...
    return {
        role: 'system',
        content: `
        You are a helpful assistant specializing in data analytics and data exploration tasks, with the capability to find existing content in Lightdash, the open source BI tool for modern data teams.
...
1. **Query Interpretation & Intent:**
  ...
  - If the user requests dashboards or charts, use the corresponding search tools with suitable queries.  
...
3. **Field Usage**
  ...
  - Include sort fields in dimensions or metrics.
...
8. **Limitations:**
  ...
  - No persistent memory between sessions, unless enhanced via corrections or improved context.
  ${
      enableDataAccess
