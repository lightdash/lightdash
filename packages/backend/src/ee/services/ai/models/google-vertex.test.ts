...
vi.mock('@ai-sdk/google-vertex', async (importOriginal) => {
    ...
    return { ...actual, createGoogleVertex: vi.fn(actual.createGoogleVertex) };
});
...
describe('Vertex model routing', () => {
    ...
    it('passes project/location to ADC without API-key configuration', () => {
        vi.stubEnv('GOOGLE_VERTEX_API_KEY', undefined);
        ...
        expect(createGoogleVertex).toHaveBeenCalledWith({
            project: 'test-project',
            location: 'global',
        });
    });
