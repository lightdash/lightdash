...
export class AiAgentToolsService extends BaseService {
    ...
    private static getSpaceMetadata(
        ...
        const breadcrumbs: FindContentSpaceBreadcrumb[] = pathParts.map(
            (_part, index) => {
                const path = pathParts.slice(0, index + 1).join('.');
                const breadcrumbSpace = spaces.find(
                    (candidate) => candidate.path === path,
                );
                return {
                    uuid: breadcrumbSpace?.uuid ?? path,
                    name: breadcrumbSpace?.name ?? pathParts[index],
                    slug: getContentAsCodePathFromLtreePath(path),
                };
            },
        );
            ...
            async () => {
                ...
                const contentResults = content.flatMap(
                    (item): FindContentResult[] => {
                        ...
                        const spaceMetadata =
                            AiAgentToolsService.getSpaceMetadata(space, spaces);
                        if ('charts' in item) {
                            return [
                                {
                                    ...item,
