...
export class ManagedAgentService extends BaseService {
    ...
    private getClient(): ManagedAgentClient {
        if (!this.client) {
            const { anthropicApiKey, serviceAccountPat, sessionTimeoutMs } =
            ...
            if (!serviceAccountPat) {
                throw new Error(
                    'MANAGED_AGENT_SERVICE_ACCOUNT_PAT is required for managed agent',
                );
            }
...
`;
    ...
    private async handleCreateContent(
        ...
        // Validate the chart payload before writing to DB
        ManagedAgentService.validateChartPayload(
