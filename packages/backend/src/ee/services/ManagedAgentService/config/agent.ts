...
export const managedAgentConfig: AgentCreateParams = {
    ...
    tools: [
        ...
        {
            description:
                'Log an actionable observation about content or the project. Examples: a chart is very popular but not pinned, popular content is in a private space, a custom metric is duplicated across many charts and should be promoted to dbt.',
            input_schema: {
                properties: {
                    description: {
                        description:
                    ...
                    metadata: {
                        description:
                            'Supporting data (e.g., views_count, unique_viewers, space_name, or governance metadata for project-level insights)',
                        type: 'object',
                    },
                    target_name: {
                    ...
                    target_type: {
                        description:
                            'Type of target. Use "project" for project-wide insights such as governance findings.',
                        enum: ['chart', 'dashboard', 'project'],
                        type: 'string',
                    },
                    target_uuid: {
                        description:
                            'UUID of the target. For project-level insights, this is the project UUID.',
                        type: 'string',
                    },
                },
                required: [
        ...
        {
            description:
                'Find custom metrics defined on saved charts that are repeated across 3 or more charts (the same metric name with the same SQL). Each finding represents a candidate to promote into a dbt model so the BI layer stops carrying duplicated logic. Returns groups of charts that share each repeated definition, with chart UUIDs, names, and the SQL of the metric. Already-flagged active findings are filtered out — only new findings are returned.',
            input_schema: {
                properties: {},
                required: [],
                type: 'object',
            },
            name: 'find_repeated_custom_definitions',
            type: 'custom',
        },
    ],
};
