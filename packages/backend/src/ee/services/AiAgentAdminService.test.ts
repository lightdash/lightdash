...
describe('AiAgentAdminService review notification settings', () => {
    ...
    it('exports existing open findings when Linear is first enabled', async () => {
        const createLinearIssues = vi.fn().mockResolvedValue(undefined);
        const listUnlinkedReviewItemsForLinearExport = vi
            .fn()
            .mockResolvedValue([
                { fingerprint: 'fp-1', projectUuid: PROJECT_UUID },
                { fingerprint: 'fp-2', projectUuid: PROJECT_UUID },
                { fingerprint: 'fp-3', projectUuid: OTHER_PROJECT_UUID },
            ]);
        const service = makeService({
            aiAgentReviewClassifierModel: {
                listUnlinkedReviewItemsForLinearExport,
            },
            aiAgentReviewNotificationModel: {
                getLinearRouting: vi.fn().mockResolvedValue({
                    organizationUuid: ORGANIZATION_UUID,
                ...
                upsertLinearRouting: vi.fn().mockImplementation((value) =>
        ...
        expect(createLinearIssues).toHaveBeenCalledTimes(2);
    ...
    it('does not re-export when Linear routing is already enabled', async () => {
        const createLinearIssues = vi.fn().mockResolvedValue(undefined);
        const service = makeService({
            aiAgentReviewNotificationModel: {
                getLinearRouting: vi.fn().mockResolvedValue({
        ...
        expect(createLinearIssues).not.toHaveBeenCalled();
    });
