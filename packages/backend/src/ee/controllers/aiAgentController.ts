...
export class AiAgentController extends BaseController {
    ...
    async listMcpServers(
        ...
        return {
            status: 'ok',
            results: await this.getAiAgentService().listMcpServers(
                toSessionUser(req.account),
                projectUuid,
            ),
        };
    ...
    async createMcpServer(
        @Request() req: express.Request,
        @Path() projectUuid: string,
        @Body() body: ApiCreateAiMcpServer,
    ): Promise<ApiAiMcpServerResponse> {
        assertRegisteredAccount(req.account);
        this.setStatus(201);
        return {
            status: 'ok',
            results: await this.getAiAgentService().createMcpServer(
                toSessionUser(req.account),
                projectUuid,
                body,
            ),
    ...
    async listAgentMcpServers(
        ...
        return {
            status: 'ok',
            results: await this.getAiAgentService().listAgentMcpServers(
                toSessionUser(req.account),
                projectUuid,
                agentUuid,
            ),
    ...
    @SuccessResponse('200', 'Success')
    @Patch(
        '/{agentUuid}/artifacts/{artifactUuid}/versions/{versionUuid}/savedChart',
    )
    @OperationId('updateArtifactVersionSavedChart')
    async updateArtifactVersionSavedChart(
        ...
        await this.getAiAgentService().updateArtifactVersionSavedChart(
            toSessionUser(req.account),
            {
                agentUuid,
                artifactUuid,
