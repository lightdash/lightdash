...
describe("DuckdbWarehouseClient", () => {
  ...
  it("should serialize shared S3 secret bootstrap across concurrent sessions", async () => {
    ...
    expect(
      runMock.mock.calls.filter(([sql]) =>
        (sql as string).includes("CREATE OR REPLACE SECRET __lightdash_s3"),
      ),
    ).toHaveLength(1);
  ...
  describe("SQL security validation", () => {
    ...
    it("should reject queries with current_setting()", async () => {
      ...
      const client = new DuckdbWarehouseClient();
      await expect(
        client.runQuery("SELECT current_setting('s3_secret_access_key')"),
      ).rejects.toThrow(
    ...
    it("should reject ATTACH statements in runSql", async () => {
      ...
      const client = new DuckdbWarehouseClient();
      await expect(client.runSql("ATTACH DATABASE 'file.db'")).rejects.toThrow(
    ...
    it("should reject LOAD statements in runSql", async () => {
      ...
      const client = new DuckdbWarehouseClient();
      await expect(client.runSql("LOAD 'malicious_extension'")).rejects.toThrow(
