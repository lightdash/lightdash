...
- **A parked lease** — `parked` means the migrator exhausted its retries and stopped on purpose. `migrate status` shows the park's `class`. A `deterministic` park (or `unknown`, from an older runner) refuses to re-run the same app version: report the failing migration and error, and point to the runbook's recovery section. Retrying is the operator's call once the cause is fixed or a corrected version is deployed. A `transient` park (lock timeout or deadlock) clears itself: a pod of the same version retries after a cool-off (2 minutes by default, `MIGRATION_TRANSIENT_PARK_COOLOFF_MS`). Report the blocking sessions from the `MIGRATION_LOCK_HOLDER` log lines instead of unlocking.
