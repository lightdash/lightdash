...
2. **Start the snippet with the key it extends.** Its first line is that key at the indent it has in the file (`metrics:` under a column's `meta`, or `additional_dimensions:`), and the lines after it are the entry to add. Use it types those lines directly under the last line in the file that is that key, and the compile test inserts them the same way, so the card shows the learner the path the entry takes. The build refuses a lesson whose key is missing from the file or belongs to a different column than the lesson declares.
