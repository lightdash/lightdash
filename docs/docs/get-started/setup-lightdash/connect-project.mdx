...
To start Lightdash with the option to connect to a local dbt project, you must specify the directory of the dbt when 
...
In the Lightdash project connection page, leave the local project directory as `/usr/app/dbt` (this is a quirk of 
docker, even if it makes little sense!).
