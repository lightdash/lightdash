...
You can find the location of your  [in your dbt `profiles.yml` file](https://docs.getdbt.com/reference/warehouse-profiles/bigquery-profile#dataset-locations), or in your BigQuery console.
...
If you need to provide access to data across multiple BigQuery projects, the service account will need to granted `roles/bigquery.dataViewer` on each additional BigQuery project.
