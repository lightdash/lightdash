...
### Ggo ahead and try adding a metric to a model in your project. 
...
2. [Via **environment variables**](#environment-variables) to use a personal access token and help automate CI&CD pipelines. _(Note that if you are using Google SSO login you should authenticate via environment variables.)_
