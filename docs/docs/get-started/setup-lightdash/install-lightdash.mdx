...
If you are using the install script, when you are asked **how do you want to setup Lightdash ?**, select the option  **with local dbt** and then provide the absolute path to your dbt project.
If you are installing manually, set the env var **DBT_PROJECT_DIR** with the absolute path to your dbt project.
:::
