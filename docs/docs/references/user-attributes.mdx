...
User attributes can be assigned to users or groups by navigating to **Organization Settings** > **User Attributes** and 
...
If a user is assigned a value for an attribute and they also belong to a group that's been assigned a different value for the same attribute, the value for the user will take precedence over the value for the group.
