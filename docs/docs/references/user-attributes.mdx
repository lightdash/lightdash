...
The `${lightdash.attributes.fruit}` values above will equal `coconut`. User attribute values take precedence over group attribute values.
...
The `${lightdash.attributes.fruit}` values above will equal either `kiwi` or `orange. The order of multiple group attribute values is not defined. 
