...
### Passing external variables to user attributes

For embedded content, you can either set user attributes to constant values, or you can adjust the value of the user attribute based on a variable coming from your app. 

For example, if I want users in my app to only see data filtered to their company, I can use a variable user attribute. 

To do this, I would create a user attribute in Lightdash called `company_name` and set some default (e.g. `null`) then, I would pass the Lightdash an external value for company name in the embed code snippet, like:

```node
import jwt from 'jsonwebtoken';
const LIGHTDASH_EMBED_SECRET = 'secret'; // replace with your secret
const projectUuid = '21eef0b9-5bae-40f3-851e-9554588e71a6';
const userCompanyName = await getUserCompanyNameFromDatabase();
const data = {
    content: {
        type: 'dashboard',
        dashboardUuid: 'a392ed6d-4c53-4102-89b7-1c57a87df391',
    },
    userAttributes: {"company_name": userCompanyName},
};
