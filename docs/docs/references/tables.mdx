...
The filter will only be applied when querying this table directly. If the current table (Table A) is included via a join to another table (Table B) then the `sql_filter` for Table B will be applied but not for Table A.
