...
| [time_intervals](#time-intervals)                 | No       | `'default'` or `OFF` or an array[] containing elements of [date](https://docs.lightdash.com/references/dimensions/#date-options), [numeric](https://docs.lightdash.com/references/dimensions/#numeric-options) or [string](https://docs.lightdash.com/references/dimensions/#string-options) options | `'default'` (or not setting the `time_intervals` property) will be converted into `['DAY', 'WEEK', 'MONTH', 'YEAR']` for dates and `['RAW', 'DAY', 'WEEK', 'MONTH', 'YEAR']` for timestamps; if you want no time intervals set `'OFF'`.                                     |
