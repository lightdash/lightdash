...
You can define multiple dimensions off a single column. This is useful when handling JSONB columns or have different
formats options.

All the [dimension configuration](#dimension_configuration), beside `time_intervals`, are available for additional
dimensions.
