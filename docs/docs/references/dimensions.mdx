...
An additional dimension is not included in yoru dbt .sql file, so it's not written to/stored in your data warehouse. When used in Lightdash, it just adds the dimension definition to your SQL query (so it's "created" at runtime).
