...
For example, you might define `unique_enterprise_deals` in a `deals` model by using a joined field in a filter: `${accounts.segment} = 'Enterprise'`. Then later you need to join `deals` to a `marketing_attribution` model, the `unique_enterprise_deals` metric must be excluded from the join unless you also join `accounts` to get the `accounts.segment` field. 
