...
<img src={HideJoinTableColumns} width="928" height="527" style={{display: "block", margin: "0 auto 20px auto"}}/>.

### Hiding joined tables/helper tables from the "Tables" list

Sometimes, you need to use an intermediate model to join two models together, and you don't want it to appear as a table to explore on its own in the list of Tables. 
...
But, models that are used in joins still need to be compiled in Lightdash. So, we suggest using some labels in your dbt models to help manage these "helper" join tables.
...
<img src={HideJoinedTableFromTablesList} width="996" height="492" style={{display: "block", margin: "0 auto 20px auto"}}/>.
