...
Users who have `full access` to a space can adjust other users' space permissions. They can restrict a user’s inherited project permissions in a space (e.g. you can make project editors into `can view` in a space). They can also give `interactive viewers` permissions to `can edit` in a space so they can save content to a space (but nowhere else in the project). 
