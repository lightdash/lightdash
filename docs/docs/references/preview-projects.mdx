...
For example, if I have a project called `Lightdash Analytics` and I create a preview of that project called `Preview Lightdash Analytics`, then `Preview Lightdash Analytics` will have the same project connection settings (warehouse connection, dbt project connection, etc.) as `Lightdash Analytics`.
...
You can either set up a single project connection that's used in every pull request, or you can [create a project connection per developer in GitHub](/guides/cli/how-to-use-lightdash-preview#how-to-use-the-developer-credentials-in-your-preview-project) (similar to `dev` profiles in your dbt project).
