...
3. [create a project connection per developer in GitHub](/guides/cli/how-to-use-lightdash-preview#how-to-use-the-developer-credentials-in-your-preview-project) (similar to `dev` profiles in your dbt project) so the connection changes depending on who opened the pull request. Tis is useful if you use different development schemas for team members developing in dbt.
...
You can also delete preview projects created from the CLI in the [Lightdash App](/references/preview-projects#lightdash-app-2). 

For more details on the `lightdash preview` CLI command, check out the docs [here](/references/lightdash-cli#lightdash-preview). 

### Lightdash App

You can delete preview projects you've created usingin the app by navigating to the `project settings`, then clicking on `all projects` in the sidebar under the `organization settings` header.
...
You can also delete preview projects created from pull requests in the [Lightdash App](/references/preview-projects#lightdash-app-2).  