...
| -------------- | --------- | -------------------------------------------------------------------------------------------------- |
| `[URL]`        | 🟢 Yes    | The URL for your Lightdash instance                                                                |
| `--token`      | 🔴 No     | For logging in via access token - common for SSO users - exclude this to login with email/password |
...
| Command Option | Required?   | Description                                                                                       |
| -------------- | ----------- | ------------------------------------------------------------------------------------------------- |
| `--name`       | 🟢 Yes      | Set the project non-interactively by passing a project name                                        |
| `--uuid`       | 🟢 Yes      | For logging in via access token - most common for SSO users - exclude to login with email/password |
...
Compile Lightdash resources using your local project and database credentials. dbt gets compiled first, then your Lightdash explores.
...
| <div style={{width:'300px'}}>Command Option</div> | Description                                                                                                |
| ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
...
This command is usually [used in Github Actions](/guides/cli/how-to-use-lightdash-deploy#automatically-deploy-your-changes-to-lightdash-using-a-github-action) or other deploy scripts. It is not common practice to use `lightdash deploy` to push your local changes up to Lightdash Cloud outside of your initial project creation or times when you need to skip your standard deploy process.
...
- `--ignore-errors`
  - Allows deploy with errors on compile (default: false)
- `--start-of-week [number]`
  - Specifies the first day of the week (used by week-related date functions). 0 (Monday) to 6 (Sunday)
- `--skip-dbt-compile`
...
Supports most (but not all) [dbt options](#dbt-options). These options are supported by `lightdash generate`:
...
All standard [dbt options](#dbt-options) work with `lightdash dbt run`.
