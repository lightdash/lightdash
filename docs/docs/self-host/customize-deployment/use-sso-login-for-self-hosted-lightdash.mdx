...
### Automatically assigning Okta users to groups in Lightdash

Once you have configured Lightdash to use Okta, users will automatically be assigned to the same groups in Lightdash as they are in OKTA. If you want to disable this functionality, you can set the following environment variable:

| Variable                  | Description                               | Required? |
| ------------------------- | ----------------------------------------- | --------- |
| `AUTH_DISABLE_GROUP_SYNC` | If `"true"` disables group sync with Okta |           |
