...
::: info
If you're running into an error: `fatal: Invalid --project-dir flag. Not a dbt project. Missing dbt_project.yml file` this is because we've assumed your dbt project is in your root folder. (e.g. `my-repo/models`)

If your dbt project is in another folder, (e.g. `my-repo/dbt/models`), then you need to update the `--project-dir` on your run command, like this:
...
# UPDATED
run: lightdash deploy --project-dir ./dbt --profiles-dir . --profile prod || lightdash deploy --project-dir ./dbt --profiles-dir .
```
