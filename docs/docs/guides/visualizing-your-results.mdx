...
import PivotTable from './assets/pivot-table-config.jpg';
...
To pivot your table, you want to move the dimension you want to pivot by to the `column` section of your table configuration. This will change the dimension from having its values populate the rows values of your table, to having it populate the column values of your table. 
...
<img src={ConditionalFormatting} width="698" height="297" style={{display: "block", margin: "0 auto 20px auto"}}/>

When you add a new rule, you’ll first need to pick which column should be affected, then you'll need to choose condition you want to match and the colour to change the cell if it matches that condition.

<img src={ConditionalFormattingRule} width="349" height="563" style={{display: "block", margin: "0 auto 20px auto"}}/>
...
To pivot by a column in your table, just add the column you want to pivot by to the `group` option in the config panel. This will rotate the rows from this dimension into columns. You can pivot by up to 3 columns in your table.

<img src={PivotTable} width="648" height="844" style={{display: "block", margin: "0 auto 20px auto"}}/>

Note that pivoted tables have a limit of 60 columns that can be displayed.
