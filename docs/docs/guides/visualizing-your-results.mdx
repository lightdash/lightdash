...
You can add subtotals to your tales by selecting `Show subtotals` in the chart configuration panel. 
...
Subtotals are only applied to `count` and `sum` metrics (other metric types or table calculations are not supported yet).

To use subtotals, you need to have at least 2 or more dimensions in your table visualization. You also cannot use subtotals on a pivoted table. 
