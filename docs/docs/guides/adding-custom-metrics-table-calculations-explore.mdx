...
<img src={EditDeleteCalc} width="592" height="268" style={{display: "block", margin: "0 auto 20px auto"}}/>
...
A lot of commonly used table calculations rely on something called [Window Functions](https://www.sqltutorial.org/sql-window-functions/). The specific way of writing these functions will depend on your data warehouse, but normally they look something like this:
...
company_name,
date,
SUM(amount) OVER(PARTITION BY company_name ORDER BY date ASC) AS running_total_amount_per_company
```
...
```sql
SUM(${table.field_I_want_to_sum}) OVER(ORDER BY ${table.field_I_want_to_order_by}, ${table.optional_other_field_I_want_to_order_by})
```