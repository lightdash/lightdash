...
Alerts let you send a saved chart the conditions on the latest value are met, and their results to people via email or Slack - even to people who don't have an account in your Lightdash instance.
...
Only users with [admin, developer or edit access](/references/roles.mdx) can create alerts. To create an alert, click on the three-dot menu in the top-right corner of your chart or dashboard, then click on `Alerts`.
...
-   **Conditions**: Configure the condition that is going to be evaluated against the firt result(s).
