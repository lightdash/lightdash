...
For example, if we had a file called `users.yml` in dbt that looked something like the one below, we'd see a Table called `users` in Lightdash.
...
Our internal dbt repo at Lightdash (`lightdash-analytics`), looks something like this:
...
We've found that this makes it easier to navigate through your `.yml` files and generally maintain your dbt models, especially as your project becomes bigger. 
...
Once you have your `.yml` files all ready to go and synced to the branch that your Lightdash project is connected to, you can just hit `refresh dbt` and your Tables should pop up in your project. 
...
<img src={ProjectManagement} width="997" height="708" style={{display: "block", margin: "0 auto 20px auto"}}/>
<img src={TablesConfiguration} width="1779" height="913" style={{display: "block", margin: "0 auto 20px auto"}}/>
...
🔥 **Hot tip**: if you want to manage the models you're using in Lightdash **_in your dbt project_**, then you can use [dbt's tags](https://docs.getdbt.com/reference/resource-configs/tags) to tag all of your models with `lightdash`. 
...
Lightdash still has access to all of the Tables from your dbt project, even if you've filtered them from appearing in the sidebar. You can access these hidden table by just clicking on the `Show hidden tables` toggle in the Tables sidebar. 

This will let you search through and explore all of the tables from your dbt project (with `.yml` files) even if they've been hidden from the main view in Lightdash. 
