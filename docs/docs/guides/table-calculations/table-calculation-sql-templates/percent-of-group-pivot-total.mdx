...
<img src={PercentOfTotalGroup} width="878" height="410" style={{display: "block", margin: "0 auto 20px auto"}}/>

And here's the SQL used in the table calculation:

```sql
${orders.total_order_amount} / SUM(${orders.total_order_amount}) OVER(PARTITION BY ${orders.status})
```
