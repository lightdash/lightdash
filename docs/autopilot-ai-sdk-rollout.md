...
Setup shows the resolved provider, model and whether the key comes from organization settings or the instance. Each completed or interrupted model loop includes that attribution in its stored summary. Usage telemetry includes the run UUID, project, organization, provider, model and key management. Scheduled runs can make multiple calls and incur token charges on that key.
