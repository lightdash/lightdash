...
4. **v1 lock + error-mapping parity** — shared
   `packages/backend/src/ee/models/slackThreadWrites.ts`.
...
Exercised indirectly by G1.7: the synthetic v1 thread took the shared
`slackThreadWrites` path and produced the archived notice exactly once, then went
silent. No v1-specific failures anywhere in the run.
