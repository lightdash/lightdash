...
Manual query downloads include `query_id`. The Export Events Explore joins
that identifier to Query Events so `chart_id`, `dashboard_id` and
`explore_name` are available when the original query reported them. Other
export paths leave `query_id` and the joined content identifiers null rather
than guessing which content produced the file.
...
Metadata includes org/project/actor/query IDs, time, format, context, and
optional job, table and row-count fields. Rows, SQL, filenames, URLs and errors
are excluded.
