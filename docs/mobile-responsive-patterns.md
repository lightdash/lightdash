...
- Dashboard and saved-chart headers use a shared phone grid: title left, actions right, metadata below, with a 16px horizontal inset. Long titles wrap to two lines; favorites move into the overflow menu. Header actions use plain icons inside 44px touch areas. Dashboard refresh becomes one refresh-options menu containing Refresh now and auto-refresh intervals. Desktop retains its split control. The pre-aggregate bolt is omitted from the phone header.
