...
- *Date-zoom cast (§1)* — **not applied.** Date-zoom dims stay on the old `TIMESTAMP` round-trip (display stays correct via the existing shift; no off-by-one); casting them for raw-value consistency is **GLITCH-505**.
