...
**Sub-day grains skip the shift entirely (GLITCH-449).** `SECOND` / `MINUTE` / `HOUR` axes are *not* shifted. The wall-clock shift is non-injective across a DST transition — at a fall-back the two 1 AM hours collapse onto one x-coordinate, at a spring-forward a phantom empty slot opens at the skipped hour — so sub-day grains keep their **raw UTC instants** on the (still `useUTC: true`) time axis and pin the gridlines instead. `getSubDayTimeAxisConfig` emits `axisTick.customValues` + `axisLabel.customValues` at the distinct bucket instants present in the data, with a custom formatter (`formatSubDayAxisLabel`) that renders each tick in the project zone, mirroring ECharts' own leveled time formatter. Positions stay injective (no DST collision or phantom gap) and ticks land on real buckets for any offset, including fractional ones. `resolveAxisTimezone` tags these `mode: 'sub-day-format'` and uses the **no-shift** formatter pair (`timezone = resolvedTimezone`, `displayTimezone = undefined`). Known limit: ticks are one-per-bucket with no adaptive thinning, so very high-cardinality sub-day axes can crowd (`gap-echarts-dst` follow-up).
...
| Sub-day axis (raw instants + `customValues`) | `resolvedTimezone` | `undefined` | Real UTC instants; positions stay injective and the custom tick formatter labels them in-zone |
| No shift (category / UTC)  | `resolvedTimezone` | `undefined`        | Values are still UTC instants — formatter converts and labels normally  |

**Skipped for:** sub-day grains (`SECOND`, `MINUTE`, `HOUR`) — they take the raw-instant + `customValues` path above, not the companion-column shift; calendar `DATE` dimensions and `convert_timezone: false` dims — not shiftable instants (the same `shouldShiftItemTimezone` opt-out the table and exports use); category-axis intervals (`WEEK`, `MONTH`, `QUARTER`, `YEAR`) — rendered as strings, not numeric positions; UTC or unresolved timezone — shift would be a no-op; pivot metadata (legend labels, stack totals) — those go through the formatter path, not the ECharts time scale.
