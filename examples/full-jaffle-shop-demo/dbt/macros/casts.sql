...
{% macro cast_numeric(column) %}
  {% if target.type == 'trino' or target.type == 'athena' or target.type == 'doris' %}
...
{# Cast to timestamp - Doris has no TIMESTAMP type, use DATETIME #}
{% macro cast_timestamp(column) %}
  {% if target.type == 'doris' %}
...
{# Cast to integer - Doris uses INT #}
...
{# Cast to time - Athena/Doris don't support TIME type, use string #}
...
{# Cast to json - Athena/Doris use JSON type but different syntax #}
...
{% macro date_diff_days(date1, date2) %}
  ...
  {% elif target.type == 'doris' %}
    DATEDIFF(CAST({{ date1 }} AS DATE), CAST({{ date2 }} AS DATE))
  {% else %}
...
{% macro timestamp_diff_days(ts1, ts2) %}
  ...
  {% elif target.type == 'doris' %}
    TIMESTAMPDIFF(DAY, CAST({{ ts2 }} AS DATETIME), CAST({{ ts1 }} AS DATETIME))
  {% else %}
...
{% macro age_years(date_col) %}
  ...
  {% elif target.type == 'doris' %}
    TIMESTAMPDIFF(YEAR, CAST({{ date_col }} AS DATE), CURRENT_DATE())
  {% else %}
...
{% macro unsupported(feature_name) %}
  {% if target.type == 'trino' or target.type == 'athena' or target.type == 'doris' %}
    NULL {# {{ feature_name }} not supported in Athena/Trino/Doris #}
  {% else %}
    {{ caller() }}
  {% endif %}
{% endmacro %}
