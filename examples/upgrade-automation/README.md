...
`/api/v1/readyz` certifies the schema gate and reports migration run ledger warnings without the per-request database work performed by `/api/v1/health`. A parked migration or an unavailable ledger is advisory and does not make the probe fail. The probe does not expose a version, so the action separately reads the public `Lightdash-Version` header from the instance root. A missing or ingress-stripped header fails closed. Old pods can return 404 for `readyz`; polling continues until the new pods answer or the verification budget expires.
