...
RUN pnpm --filter @lightdash/common build

# Build warehouses
COPY packages/warehouses/tsconfig.json ./packages/warehouses/
COPY packages/warehouses/src/ ./packages/warehouses/src/
RUN pnpm --filter @lightdash/warehouses build

# Build backend
COPY packages/backend/tsconfig.json ./packages/backend/
COPY packages/backend/src/ ./packages/backend/src
RUN pnpm --filter @lightdash/backend build

# Build frontend
COPY packages/frontend ./packages/frontend
RUN pnpm --filter @lightdash/frontend build
