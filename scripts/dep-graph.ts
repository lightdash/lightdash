...
const DUPLICATION_CACHE = path.join(__dirname, '.dep-graph-duplication.json');
...
function runDuplicationAnalysis(graph: GraphData, force: boolean): void {
    const hash = computeDuplicationHash(graph);
    const shortHash = hash.slice(0, 12);

    if (!force && fs.existsSync(DUPLICATION_CACHE)) {
        try {
            const cached = JSON.parse(fs.readFileSync(DUPLICATION_CACHE, 'utf-8'));
            if (cached._hash === hash) {
                console.log(`Duplication cache up to date (${shortHash}…). Use --refresh to rescan.`);
            ...
            console.log(`Source files changed since last duplication scan (${shortHash}…). Rescanning...`);
        } catch {
            console.log(`Duplication cache corrupt. Rescanning...`);
        }
    } else if (force) {
        console.log(`Forced duplication rescan (${shortHash}…)...`);
    } else {
        console.log(`No duplication cache found (${shortHash}…). Scanning...`);
    }
    ...
    for (const [nodeId, stats] of perNodeDup) {
        ...
        const dup: NodeDuplication = {
            clonedLines: stats.clonedLines,
            totalLines: stats.totalLines,
            ratio,
        };
        perNodeResult[nodeId] = dup;
        const node = graph.nodes.find(n => n.id === nodeId);
        if (node) {
            node.duplication = dup;
        }
    }
    ...
    fs.writeFileSync(DUPLICATION_CACHE, JSON.stringify(cacheData, null, 2) + '\n');
    console.log(`Cached to ${DUPLICATION_CACHE}`);
}

// ---------------------------------------------------------------------------
// Duplication summary (via Claude CLI)
// ---------------------------------------------------------------------------

const DUPLICATION_SUMMARY_CACHE = path.join(__dirname, '.dep-graph-duplication-summaries.json');

function summarizeDuplication(graph: GraphData, force: boolean): void {
    ...
    if (!force && fs.existsSync(DUPLICATION_SUMMARY_CACHE)) {
        try {
            const cached = JSON.parse(fs.readFileSync(DUPLICATION_SUMMARY_CACHE, 'utf-8'));
            if (cached._hash === hash) {
                console.log(`Duplication summary cache up to date (${shortHash}…). Use --refresh to regenerate.`);
            ...
            console.log(`Duplication data changed since last summary (${shortHash}…). Regenerating...`);
        } catch {
            console.log('Duplication summary cache corrupt. Regenerating...');
        }
    } else if (force) {
        console.log(`Forced duplication summary regeneration (${shortHash}…)...`);
    } else {
        console.log(`No duplication summary cache found (${shortHash}…). Generating...`);
    }
...
Return as JSON with "pairAdvice" (keyed by "nodeA\\0nodeB" with names alphabetically sorted) and "nodeSummaries" (keyed by node name).`;
    ...
    const pairAdvice: Record<string, string> = output.pairAdvice;
    const nodeSummaries: Record<string, string> = output.nodeSummaries;

    for (const edge of dupEdges) {
        const sorted = [edge.from, edge.to].sort();
        const advice = pairAdvice[sorted.join('\0')] || pairAdvice[sorted.join(' ')];
        if (advice) {
            edge.duplication!.advice = advice;
        }
    }
    for (const node of dupNodes) {
        if (nodeSummaries[node.id]) {
            node.duplicationSummary = nodeSummaries[node.id];
        }
    }

    console.log(`Got advice for ${Object.keys(pairAdvice).length} pairs, ${Object.keys(nodeSummaries).length} nodes.`);
    ...
    fs.writeFileSync(DUPLICATION_SUMMARY_CACHE, JSON.stringify(cacheData, null, 2) + '\n');
    console.log(`Cached to ${DUPLICATION_SUMMARY_CACHE}`);
}
