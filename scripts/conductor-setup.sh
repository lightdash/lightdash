...
if [ -f "$ENV_FILE" ]; then
    ...
    sed -i '' '/^LD_INSTANCE_ID=/d; /^PGHOST=/d; /^PGPORT=/d; /^PGUSER=/d; /^PGPASSWORD=/d; /^PGDATABASE=/d; /^PORT=/d; /^FE_PORT=/d; /^SCHEDULER_PORT=/d; /^DEBUG_PORT=/d; /^SDK_TEST_PORT=/d; /^SPOTLIGHT_PORT=/d; /^LIGHTDASH_PROMETHEUS_PORT=/d; /^SITE_URL=/d; /^INTERNAL_LIGHTDASH_HOST=/d' "$ENV_FILE"
    cat >> "$ENV_FILE" <<ENVEOF
